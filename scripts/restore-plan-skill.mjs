#!/usr/bin/env node
/** Dev-only: restore skills/feature/plan/SKILL.md from git HEAD + scope migration */
import { execSync } from 'child_process';
import fs from 'fs';
import path from 'path';

const ROOT = path.resolve(import.meta.dirname, '..');
let body = execSync('git show HEAD:skills/2-plan/SKILL.md').toString('utf8');
...
const reps = [
  ...
  ['profiles/<project_profile.name>/skills/2-plan/', 'profiles/<project_profile.name>/skills/plan/'],
...
// consumer 布局：framework/skills/feature/plan → host 根 4 层
body = body.replace(
...
const out = path.join(ROOT, 'skills/feature/plan/SKILL.md');
