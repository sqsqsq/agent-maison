...
2. **命名业务入口**：UT/集成测试应优先调用 `coordinator` / `ui_bindings.user_actions.calls` 声明的命名方法，而不是通过 UI 手势模拟（UI 交给 device-testing 或端到端框架）。
...
与 `` `profile-skill-asset:business-ut/sample_flow_use_cases` `` 同目录下的 `use-cases.yaml` 对齐阅读。
...
| UI / 真机交互 | `acceptance.yaml` > `device_focus` | device-testing test-plan + harness |
