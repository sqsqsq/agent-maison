...
> **用途**：写 `testability-audit.md` / `mock-plan.yaml` / 单元测试源文件前必读。只列约束，不讲原理。
...
- [ ] mock-plan：纯 YAML，`ts_expr` 带 `as` 或 `new`（若 profile 使用 typed mock 表达式）
- [ ] 用例名：以 `[AC-`、`[BD-` 或 `[BRANCH-` 开头，且 ID 来自当前 feature 规约
