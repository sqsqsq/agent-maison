# `generic` · Skill `framework-init` profile addendum
...
**Personal setup（DevEco 路径）**：本 profile **不要求**在项目 init 写入宿主 IDE 路径；个人路径由阶段 `--ensure` 内联 → `framework.local.json`（registry `setup.deveco_path`）。
