...
> coding（Coding）的 git diff 不得越界到 `in_scope_modules` 之外的模块。
...
> **本节是 Harness 的门禁入口**：`impact` 字段决定 requirement-design Step 12 是否要更新 `doc/architecture.md`。
...
>
> 凡涉及架构级变更，必须在 requirement-design Step 12 同步更新 `doc/architecture.md`（及 `framework.config.json` / `module-catalog.yaml` 的相应段落）。
...
**代码形态三选一**（由 coding 编码阶段按复杂度自选，设计阶段**不强制目录**）：
...
> - `use-cases.yaml > ui_bindings[].user_actions[].calls` 列出的每个符号必须是**命名符号**（传统函数 / 类方法 / 类字段函数 `handler = () => {}` / 顶层命名 const 赋值均合法），不能是**匿名** inline lambda（直接挂到 UI 事件上）（由 coding harness 的 `named_business_handler` BLOCKER 强制）
> - **禁止**为了"便于 UT 打桩"新造 `XxxPort` 接口；`data_boundaries[].type` 必须引用 `contracts.yaml > interfaces[].class` 中已登记的现有数据层类（由 business-ut harness 的 `boundary_matches_contracts` 强制）
...
> **同时必须产出**：`doc/features/{module}/use-cases.yaml`（Schema 见 [use-cases-schema.md](../../../profiles/hmos-app/skills/business-ut/templates/use-cases-schema.md)）。
> **样例**：见本 profile 下 `skills/feature/business-ut/examples/sample-flow/`（纯规约示例，无宿主代码）。
...
#### 分支清单（与 `use-cases.yaml > branches` 1:1 对应，business-ut 据此生成 UT 用例）
