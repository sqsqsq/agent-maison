...
function runAaDumpList(deviceSn?: string): { status: number | null; output: string } {
  ...
  const r = runHdcRaw(hdcExe, args, { timeout: 4000, maxBuffer: 2 * 1024 * 1024 });
...
export function collectDeviceInfo(deviceSn?: string): DeviceInfo {
  ...
  const ver = runHdcRaw(hdcExe, ['-v'], { timeout: 5000, maxBuffer: 1024 * 1024 });
  const verOut = `${ver.stdout ?? ''}${ver.stderr ?? ''}`.trim();
  if (verOut) info.hdc_version = verOut.split('\n')[0].slice(0, 200);

  const model = runHdcRaw(
    hdcExe,
    [...hdcTargetPrefix(), 'shell', 'param', 'get', 'const.product.model'],
  ...
  const api = runHdcRaw(
    hdcExe,
    [...hdcTargetPrefix(), 'shell', 'param', 'get', 'const.ohos.apiversion'],
    { timeout: 5000, maxBuffer: 1024 * 1024 },
  );
