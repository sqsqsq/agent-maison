...
test('六轮 P1-3：占位检查按 $r 实际引用模块限定——未引用模块残留占位不入债务；引用后命中', () => {
  withTmp(root => {
    ...
    // 组1：B 无任何源文件 → B 的残留占位不得入债务（asset_placeholder_present 不触发）
    const r1 = checkVisualParity(ctx).find(r => r.id === 'asset_placeholder_present');
    assert.strictEqual(r1, undefined, `未引用模块残留占位不得计入：${r1?.details}`);
    ...
    assert.strictEqual(r2, undefined, `注释/字符串伪引用不得计入：${r2?.details}`);
