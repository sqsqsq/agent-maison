...
> 推荐 **direct 根键**：`{"touch":{"by_text":"…"}}`；预启由 harness `aa start` 完成，步骤列**不含** `start_app`。
>
...
|----------|---------|---------|---------|---------|--------|---------|
| TC-001 | 卡片列表展示 | 已启动 app（harness aa start 预启） | {"touch":{"by_text":"我的钱包"}} ; {"swipe":{"direction":"UP","distance":50,"area":{"by_type":"Scroll"}}} | 列表展示至少 1 张卡片 | P0 | AC-001 |
| TC-002 | 充值 100 元 | 已启动 app | {"touch":{"by_text":"充值"}} ; {"input":{"text":"100","by_id":"amount_input"}} ; {"touch":{"by_text":"确认"}} | Toast 显示充值成功 | P0 | AC-005 |
