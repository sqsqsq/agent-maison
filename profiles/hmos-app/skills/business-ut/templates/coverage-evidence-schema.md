...
> **business-ut 在存在 `ut_layer ∈ {unit, both}` 且 P0/P1 范围时须产出**；harness **不会**自造 `mappings[]`（仅校验，避免自证绿洞）。
...
- in-scope **P0/P1 unit/both** 缺证据 → `ut_case_per_unit_ac` / `ut_coverage_evidence_*` **FAIL**，非静默 SKIP。
- 有文件但 **mappings[] 未覆盖每条 P0/P1** → `ut_coverage_evidence_mappings_complete` **FAIL**。
- SKIP 仅当：无 P0/P1 unit/both scope、profile 禁用 UT、或已登记兼容降级（`skip_reason`）。
