# `hmos-app` · Skill `prd-design` profile addendum
...
| Visual Handoff / `ui_change`（若启用） | `framework/skills/feature/prd-design/` 正文与 `prompts/` |

### skill-assets.yaml 键

机器清单：`framework/profiles/hmos-app/skills/skill-assets.yaml`。根 `SKILL.md` 使用 `` `profile-skill-asset:prd-design/<键>` `` 指向下列文件：

| 键 | 相对 `skills/feature/prd-design/` |
|----|------------------------------|
