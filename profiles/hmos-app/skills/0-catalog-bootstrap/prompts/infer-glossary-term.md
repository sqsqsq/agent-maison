...
> **为什么必须有这步**：catalog 作者可能在同一模块的 `typical_business_terms` 和 `NOT_responsible_for` 里同时提到了同一个业务词（以钱包工程为例：`AccountManager` 既把"账户"列为 typical term，又在 NOT_responsible_for 里排除"钱包账户"）。弱模型一旦在 Step 1 命中就默认 high 通过，会漏掉这类内部冲突——这是 glossary 误分模块的**首号原因**。
