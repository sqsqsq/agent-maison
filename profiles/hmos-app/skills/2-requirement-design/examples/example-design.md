...
| F3 | 登录态展示（若 PRD 要求） | AccountManager | 04-BusinessBase | 账号会话与状态订阅 |
...
| AccountManager | 04-BusinessBase | 若 PRD 要求展示登录信息或触发登录流程 |
...
设计落地时请在 `doc/features/<feature>/design/contracts.yaml` 写入真实路径、导出符号与数据模型；此处仅示意：
...
- **本地**：草稿条目模型、最后同步时间字段。
- **远端**：`RemoteTaskGateway` 模拟返回成功/失败；未接真实环境时在 PRD 标注「模拟数据」。
