...
> 宿主细则通过 `doc/extensions/knowledge/` 章节模板（作者动笔前读；goal 模式下注入阶段 prompt）与 `phase_rules_overlays.spec`（harness 强制）叠加；`hooks/spec/on_context_load.md` 的片段只进 verifier 上下文，不会自动送达作者。
