...
// ============================================================================
// 读取 framework/specs/phase-rules/design-rules.yaml + doc/features/{feature}/design.md
...
const checker: PhaseChecker = {
  ...
  async check(ctx: CheckContext): Promise<CheckResult[]> {
    ...
    if (!design) {
      const designRel = relFeatureArtifact(ctx.projectRoot, ctx.feature, 'design.md');
      return [{
        id: 'design_file_exists', category: 'structure',
        description: `${designRel} 不存在`,
        severity: 'BLOCKER', status: 'FAIL',
        details: `设计文件 ${designRel} 不存在，无法进行任何检查。`,
