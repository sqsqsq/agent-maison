...
function resolveCapabilityAxis(args: ResolveVisionContextArgs): VisionCapabilityAxis {
  ...
  if (isVisionCanaryFresh(canary, adapter)) {
    // plan d8c5f3a7 T1：采信判据抽为共享谓词 canaryAdmissibleForRun——preflight 的
    ...
    // 注：preflight 侧 decideVisionCanaryProbe 消费同一谓词，此情形会先触发 in-run 重探；
    // 走到这里通常意味着重探未执行/未写盘（stale-if-error），保守降级即正确行为。
  }
