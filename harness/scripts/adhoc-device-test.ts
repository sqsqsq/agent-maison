...
// 设备目标**只在门之后**读（c7d2a9e4 D1）：此前这里顶层读 env，而即席链从未起过门，
// 于是 env 恒空、恢复桥"不对未知设备动手"整体跳过——登记好的凭据永远用不上。
...
// 门与设备操作都要 await：本文件是顶层脚本，commonjs/ES2020 无顶层 await，
// 故把"起门之后的全部流程"包进一个 async IIFE（缩进以外零改动）。
void (async (): Promise<void> => {
  ...
  if (dumpUiOnly) {
    ...
    // 设备入口门：ensure 之后（pip 可能半分钟且不碰设备）、任何设备命令之前
    logAdhocPhase('device_gate');
    let gate;
    try {
      gate = await applyPhaseEntryDeviceGate({
        projectRoot,
        phase: 'testing',
        startedBy: `adhoc-${process.pid}`,
        log: line => console.error(`[device] ${line}`),
      });
    } catch (err) {
      console.error(`设备策略检查执行失败：${(err as Error).message}`);
      process.exit(1);
    }
    if (!gate.ok) {
      console.error(gate.reason ?? '设备前置未通过');
      process.exit(1);
    }
  ...
  // 设备入口门：ensure 之后、`resolveMainAbilityForBundle`（执行链第一个 `bm dump`）之前。
  // 门失败后不得再发出任何设备命令。
  logAdhocPhase('device_gate');
  let gate;
  try {
  ...
  if (!gate.ok) {
    const reason = gate.reason ?? '设备前置未通过';
    console.error(reason);
    writeAdhocTracePlaceholder(traceOutPath, {
      feature: ADHOC_FEATURE,
      phase: 'testing',
      outcome: 'aborted',
      error_kind: 'device_not_ready',
      error_message: reason.split('\n')[0],
      bundle,
