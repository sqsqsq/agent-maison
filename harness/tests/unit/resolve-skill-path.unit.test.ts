...
const cases: Array<{ name: string; run: () => void }> = [
  {
    name: 'loadSkillsIndex: 16 个 builtin skill 且路径可解析',
    run: () => {
      const index = loadSkillsIndex(FRAMEWORK_DIR, true);
      assert(index.skills.length === 16, `skills.length=${index.skills.length}`);
      const ids = listBuiltinSkillIds(FRAMEWORK_DIR);
      assert(ids.length === 16, `ids.length=${ids.length}`);
      for (const id of ids) {
