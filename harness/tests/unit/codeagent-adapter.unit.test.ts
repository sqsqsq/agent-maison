...
// ============================================================================
// 覆盖：adapter.yaml 结构/跨目录引用可解析；commands ×15 归一化等值（delta 白名单）；
...
/** 普通 14 份唯一允许的 delta：身份行（含前置空行，与生成器插入形态逐字对应） */
...
const cases: Array<{ name: string; run: () => void }> = [
  {
    ...
    run: () => {
      ...
      assert.strictEqual(commandFiles.length, 15, `commands 应 15 份，实际 ${commandFiles.length}`);
  ...
  {
    name: 'commands 归一化等值：普通 14 份仅身份行 delta；goal-mode 三处 delta；其余逐字节同 claude',
    run: () => {
