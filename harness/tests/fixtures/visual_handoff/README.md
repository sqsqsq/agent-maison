...
> [`profiles/hmos-app/harness/tests/fixtures/spec/`](../../../../profiles/hmos-app/harness/tests/fixtures/spec)，单元用例在
...
| 场景 | 用例 id |
|------|---------|
| 无 `ui_change` yaml、无顶层 `spec` 段 → 静默零条结果 | `no_ui_yaml_and_no_prd_section_returns_empty_array` |
| `enforcement=strict` 且缺整块 yaml → FAIL | `missing_ui_yaml_strict_fail` |
| 仓库内相对路径不存在、无 `spec` 段 → FAIL（声明即承诺） | `unreachable_repo_relative_implicit_strict_fail` |
...
## Fixture（`spec/visual_handoff_*`，`tests/run-tests.ts`；目录 `profiles/hmos-app/harness/tests/fixtures/spec/`）

对应 plan §2.0 决策表第 1/2/4 行——通过 `EXPECTED.json.rules` 的 `must_be_absent` 与
`details_includes` 字段断言局部规则结果，**不**约束其它 spec 检查的状态：

| Fixture | 决策表行 | 验证点 |
|---------|----------|--------|
| `spec/visual_handoff_silent_no_ui_change_no_prd_section` | 第 1 行：云侧/库工程默认静默 | 无 `ui_change` + 无 `spec` 段 → 报告里**不出现** `visual_handoff*` 任一规则 |
| `spec/visual_handoff_strict_missing_ui_change_block_fail` | 第 2 行：UI 工程主动拦截缺块 | 无 `ui_change` + `spec.strict` → `visual_handoff_ui_change=FAIL/BLOCKER`，详情含「opt-in」字样 |
| `spec/visual_handoff_repo_assets_reachable_pass` | 第 4 行：声明 + 可达 → PASS | `ui_change=new_or_changed` + 仓库内可达 path → `visual_handoff=PASS`，无残留 `visual_handoff_refs` 报错 |
