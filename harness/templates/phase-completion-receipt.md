...
# .claude/hooks/check-phase-completion.mjs 解析作物理拦截依据。
#
# CLAUDE.md §5.1 SSOT：四条件缺一不可。
# 编造任意一项 → 反假设条款触发 → 任务失败。
#
...
agent_model: "<实际模型 id，如 minimax-2.5 / claude-4.6-opus / gpt-5.5>"
agent_runtime: "<claude-code-cli | cursor | other>"
...
# ----------------------------------------------------------------------
# 3. trace.json 凭证（Layer 1 凭证，CLAUDE.md §5）
# ----------------------------------------------------------------------
trace_json:
...
self_check:
  ...
  q4_evidence: "<若 q4 为 true，说明本次决策没有引用 CLAUDE.md/SKILL.md 中不存在的规则；若曾自我设限，请 quote 原文行号>"
---
...
## 反假设条款回顾（CLAUDE.md §6.5）

- [ ] 我没有引用 `CLAUDE.md` / 任一 `SKILL.md` 中**不存在**的规则作为跳过任何步骤的理由。
