...
兼容降级 **`compat.yaml`** 不是图上节点：仅在 **`prd`～`ut`**（不含 `testing`）的脚本汇总前按需生效；**全局 phase 不参与**。见 [`evolution/compat-protocol-v1.md`](evolution/compat-protocol-v1.md)。
...
| **v2.5 弱模型工作流强制门** | 步骤跳过 / 规则幻觉 | Layer 1（实例根全局入口 §4.1 授权 / §5.1 闭环 / §6 反假设条款）+ Layer 2（`framework/harness/templates/phase-completion-receipt.md` + `check-receipt.ts`）+ Layer 3（Stop hook，由具备 `hooks` 的 adapter 下发，见 [`agents/README.md`](../agents/README.md)）；schema 扩展 `settings_file` + `hooks`，由 `00-framework-init` Skill 下发 |
...
| **v2.7 hvigor 加速 + product 自动探测（hmos-app）** | hvigor 命令拼装 BLOCKER 与编译性能 | `hvigor-runner.ts` 装配 hvigor flag（`-p buildMode=debug` 仅 assembleApp / `--daemon` / `--parallel` / `--incremental` / `--analyze=advanced`，可由 `framework.config.json > toolchain.hvigor` 覆盖）；新增 `detectProduct(projectRoot)` 自动从 `framework.config.json > toolchain.preferredProduct` > `build-profile.json5 > app.products[0].name` > `'default'` 兜底，**修 v2.6 写死 `product=default` 导致 product 名为 `mirror` 工程编译失败的隐藏 BLOCKER**；`buildAssembleAppArgs` / `buildModuleHapArgs` 两个纯函数被独立 export 供单测覆盖；`detect-product` + `hvigor-args` 共 12 case 单测；本仓库小工程实测 cold 路径 -14.9%、warm 路径 +47%（反向变慢）的反例已如实写入 [`profiles/hmos-app-harness-toolchain.md` §4](profiles/hmos-app-harness-toolchain.md) 避免后续 agent 假设 "必快" |
| **v2.8 Stop hook 跨会话隔离** | CLI 重启 / 多任务切换 | 修 Layer 3 "无会话感知"导致重启 agent CLI 后被 `.current-phase.json` 遗留任务硬拦截无关问答的事故；`framework.config.json` 新增 `state_machine.{grace_period_minutes, ttl_hours, schema_version}`（默认 5min / 12h）；hook 端 `evaluateSessionStaleness` 五态分类；`harness-runner.ts --clear-state` 清理出口；全局入口 §5.1.1 + §6.5「作用域澄清」；`hook-stale-state.unit.test.ts`（详见 [`operations/harness-runbook.md §10`](operations/harness-runbook.md)） |
| **v2.8.1 全局阶段豁免**     | init 中途被 hook 拦截 | 修 runner 对全局阶段误写 state；`isGlobalPhase` skip + hook 对 `init/catalog/glossary/docs` allow；全局入口 §5.1 声明判据仅覆盖六个 feature 维度阶段；单测 T13/T14 |
| **v2.8.2 多问题决策结构化** | framework-init 多个 y/n 并排 | 修 Step 4 并排抛多项覆盖确认 + 单 `y` 歧义事故；详见 [Skill 0 Step 0.3.4](../skills/00-framework-init/SKILL.md) 与 `multi-question single-y Ban` |
| **v2.8.3 弱模型 tool-call 兜底** | `/framework-init` 渲染 entry-file 时 Write 工具反复 InputValidationError 卡死 | 修弱模型（内网 lingxi-v3 / minimax-m2.7 等）通过 LLM tool-call 协议向 Write 工具传递 200+ 行中文 markdown content 时频繁返回 `parameter file_path / content missing`、AI 重试两次仍失败、整个 init 流程卡死的事故；[Skill 0 Step 4.1](../skills/00-framework-init/SKILL.md) 重排路径优先级——`render-agents-md.mjs` 由"可选辅助"升级为**首选 / 弱模型强制路径**（脚本在 Node 进程内完成大 content 渲染 + 落盘，不经 tool-call 传递从机制上规避失败），手工 Read+Write 降级为兜底（仅脚本不可用时启用）；新增 BLOCKER 条款 `tool-call retry-loop Ban`：兜底路径失败 ≥ 2 次必须切首选脚本路径，首选脚本路径也失败必须把 exit_code/stderr 反馈用户请求人工介入，严禁循环重试 |
| **feature-compat（升级止血）** | 存量 feature 撞新版 BLOCKER | `doc/features/<feature>/compat.yaml`：可过期临时降级（仅 `prd`～`ut`）；`compat-loader` + `feature-compat.schema.yaml`；正规化路径 `npm run backfill:context`；详见 [`evolution/compat-protocol-v1.md`](evolution/compat-protocol-v1.md) 与顶层 [`README.md`](../README.md) |
...
│   ├── docs/                         对外讲解 + 演进材料（本目录）
│   ├── skills/                       生成层：9 篇阶段 Skill 正文目录（★00‑init · 0‑catalog+glossary · 1～6）
│   ├── specs/phase-rules/            阶段级规约 YAML（每 harness phase 一份，含 extensions/init/docs）
│   ├── workflows/                     可选：workflow DAG（默认 spec-driven.workflow.yaml）
│   ├── harness/                      验证层：check-*.ts + verify-*.md + harness-runner.ts
│   ├── agents/                       可插拔 adapter（列表见该目录 README）
...
│   ├── extensions/               ← 可选：实例扩展（manifest / hooks / knowledge；见 concepts/extensibility.md）
...
├── adapter 产物目录（点号开头，见各 adapter）     ← 路由 / 跳板 / 规则 / hooks
...
| 5    | [`5-business-ut`](../skills/5-business-ut/SKILL.md)                     | 业务级 UT                         | `dag.yaml` + `*.test.ets` + 可选 `ut/reports/ac-coverage.json`  | `ut_import_whitelist`、`it_drives_flow`、`branch_coverage_full`、`acceptance_coverage`、**`ut_tsc_compiles`**、**`ut_hvigor_build`**、**`ut_hvigor_test`**、**`ut_no_src_mutation`** |
...
- 每阶段产物**必须通过**：脚本 Harness（`harness-runner.ts --phase <phase>`；**功能 phase**须加 `--feature <name>`，**全局 phase**不写 `--feature`）+ AI Harness（`prompts/verify-<phase>.md`，仅功能 phase / 适用时由 verifier 子 agent 执行）；phase 表见 **[`operations/harness-runbook.md`](operations/harness-runbook.md)**。
...
- **渐进增强确认**（[user-confirmation-ux.md](../skills/reference/user-confirmation-ux.md)）：widget + portable 编号菜单 + 决策复述；裸「好/继续」不构成决定。registry：[confirmation-registry.yaml](../skills/reference/confirmation-registry.yaml)；静态 lint：`check-skills-confirmation-ux.ts`（docs phase / `npm test`）。
...
| **acceptance→test-plan 分层** | Hylyre build/install/run 与 plan↔hylyre SSOT 已就绪；薄弱点在 acceptance `device_focus` 与 test-plan 按 `ut_layer` 派生 | 旧 todo 与三清单并存易误读 | 见 [acceptance-layering.md](concepts/acceptance-layering.md)；`device_ac_delegation` 已升 BLOCKER（`device_focus`） |
...
- **验收分层存量迁移**：各 feature 补全 `device_focus`、删除 `device-testing-todo.md`、按 acceptance 重派生 test-plan
