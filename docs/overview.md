...
| **v2.8.1 全局阶段豁免**     | `/framework-init` 中途被 hook 拦截 | 修 v2.8 主体回归：runner 之前对**所有** phase 都写 `.current-phase.json`，导致全局阶段（init / catalog / glossary / docs，无 feature 维度，没有完成回执模板）被 Stop hook 当成"未闭环 feature 阶段"硬拦截，要求填写**根本不存在**的 `doc/features/_global/init/phase-completion-receipt.md`；修复双保险——`harness-runner.ts > writeCurrentPhaseState` 在 `isGlobalPhase(phase)` 时直接 `return` 不写 state，`.claude/hooks/check-phase-completion.mjs > evaluateState` 看到 `state.phase ∈ {init, catalog, glossary, docs}` 时 `allow=true`（兜底历史污染）；`CLAUDE.md` §5.1 显式声明"判据仅覆盖 PRD/design/coding/review/UT/device-testing 六个 feature 维度阶段"；`hook-stale-state.unit.test.ts` 增 T13/T14 共 14 case |
| **v2.8.2 多问题决策结构化** | `/framework-init` 多个 y/n 并排抛出时回执歧义 | 修 Skill 0 的多问题并排抛 + 单 `y` 含混回应事故：Step 4 同时抛"CLAUDE.md 是否覆盖"+"claude templates 5 个漂移文件是否覆盖"两个独立 y/n，用户回 `y`，AI 自由解释为"两个都覆盖"违反 §6.5 反假设条款；新增 [Skill 0 Step 0.3.4](../skills/00-framework-init/SKILL.md) 强制把所有 POPULATED 项 y/n 决策**一次性收齐**——逐项编号 `Q1` / `Q2` / `Q3.1` / `Q3.2` / …、第 3 项**逐文件**子编号严禁折叠；规定标准回复 `Q1=y Q2=n Q3.1=y …` 与全选速记 `all=y`；Q 数 ≥ 2 时裸 `y` / `好` 视为含混回复必须退化为逐题再问；Step 4 / 4.1 / 5.1 改为**只读 0.3.4 已记录决策**严禁下游再询问；新增 BLOCKER 条款 `multi-question single-y Ban` |
