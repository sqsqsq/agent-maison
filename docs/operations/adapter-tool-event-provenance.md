...
| codeagent | `codeagentcli -p`（Claude Code 内核 fork，prompt 走 stdin，flags 与 claude 等价） | 与 claude 同路线①：`--output-format stream-json --verbose` NDJSON 事件流 | **`structured_events`（已声明，2026-07-29）**——agent-invoke 按家族谓词复用 claudeArgv | **2026-07-29 宿主实采（plan c7a9e2f4 T0）**：断网 api_retry/终局 result 信封 + Read tool_use 事件（1×1 png 实读）三段确认与 claude 逐字段同构（fork 附加 `tool_use_result.vlDescription` 等扩展字段不在解析路径）；脱敏 fixture 固化于 codeagent-adapter.unit.test.ts；解析器已入册（IMAGE_READ_PARSERS.codeagent=parseClaudeImageReadEvents）。实采顺带暴露并修复哨兵两处漏判（字符串 error_status / 终局行无 api_error_status，claude 同源受益）。余：首次真 goal run 端到端观察（plan c7a9e2f4 T6） |
...
| codex | `codex exec --json`（stdin prompt；`--json` 由 codexArgv 独立追加，plan e6b3f8d2 t1） | `--json` 是 **terminal/usage 事件流**（`turn.completed`/`turn.failed` 两终态 + `turn.completed.usage`），**不是工具调用证据**：codex 无 Read 工具，本地图片注入走 `view_image`/`-i`，其事件是否稳定出现在 stdout 仍待实测；`cat` 图片≠视觉注入，不得计入 | **恒 `none`（2026-08-25 复核：`--json` 不构成升格理由）** | terminal/usage 已实证并固化 fixture（`harness/tests/unit/fixtures/codex-terminal-*.jsonl`，CLI 0.149.0）；**图片/工具证据仍待宿主实测** |
...
- [ ] codex：`view_image` 事件是否稳定出现在 exec stdout。
      （注：`exec --json` 已于 2026-08-25 实证并接入，但它只提供 **terminal 终态与 usage**——
      plan e6b3f8d2 t1 明确：stdout 有 terminal JSONL ≠ 工具调用可审计，`tool_event_provenance`
      保持 `none`，codex 不进 `IMAGE_READ_PARSERS` 注册表，也不签发 verified critic 回执。）
