...
- (a) 类靶的自动拦截主责在 spec 合同前置（t6），树侧只做 WARN 辅助——这是设计决定，
  不是实现缺口。
- critic 回执的 `input_provenance`：tool_read 型 adapter（claude/cursor）由 agent 会话内
  Read 图片，调用侧无法证明注入 → 一律如实 `unverified`；`verified` 档保留给
  native_attach 或未来 transcript 验读增强。
