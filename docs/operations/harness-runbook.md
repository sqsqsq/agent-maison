...
| ut      | `ut_hvigor_build`     | `buildUtHvigorArgs` → 与 DevEco「Run ohosTest」对齐：`node hvigorw.js --mode module`、`-p isOhosTest=true`、`-p buildMode=test`、`genOnDeviceTestHap` + task 后 `analyze=normal`（及 parallel/incremental/daemon）；详见 `hvigor-runner.ts` |
...
### 9.4 v2.7+ hvigor 加速 flag 与 v2.9 coding 命令默认值
...
参数装配源码见 `framework/harness/scripts/utils/hvigor-runner.ts`：`buildCodingHvigorArgs` / `buildAssembleAppArgs` / `buildModuleHapArgs`（default 目标）/ **`buildUtHvigorArgs`（ohosTest，DevEco 对齐）**。
...
默认值对齐内网真实工程常用命令；如真实工程存在自定义 `onlineSign` / `archivePackage` / 签名产物改名等逻辑，可通过 `framework.config.json > toolchain.hvigor` 做 A/B：
...
| Flag                  | 收益来源                                                                  | 风险点                                                                            |
| --------------------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `-p product=<detect>` | 内网工程 product 名非 `default` 时需正确注入 | 自动探测：`toolchain.preferredProduct` > `build-profile.json5` 中优先命中名为 `product`/`default` 的项 > 首位 > 兜底 `default` |
