...
| 一半业务一半 UI 的验收项 | 业务部分进 Skill 5；UI 部分在 Skill 1 的 `acceptance.yaml` > `device_focus`，由 Skill 6 派生 test-plan |
...
真机要点 SSOT 在 **Skill 1** 的 `acceptance.yaml` > `device_focus`（`ut_layer ∈ {device, both}`），**不再**产出 `device-testing-todo.md`。
...
flowchart TD
  ...
  DevFocus["acceptance.yaml<br/>device_focus（Skill 1）"]
...
- **`device_focus`**：在 Skill 1 写入 acceptance；Skill 6 据此派生 test-plan（见 [acceptance-layering.md](../concepts/acceptance-layering.md)）。
...
| --- | --- |
| `unit` | 必须产出 UT |
| `both` | 业务部分必须产出 UT；UI 部分在 acceptance `device_focus`（Skill 1），由 Skill 6 派生 test-plan |
...
UI 导航、Toast、弹窗不要画成 UT assertion。它们应进入 `ui_subscription`（文档化）或 Skill 1 的 `acceptance.yaml` > `device_focus`。
...
### Step 5：真机要点（不在 Skill 5 新建平行清单）

`device` / `both` 的真机可观察要点应在 **Skill 1** 写入 `acceptance.yaml` > `device_focus`（`both` 须拆分 `ut_focus` + `device_focus`）。Skill 5 **不再**产出 `device-testing-todo.md`。

若发现缺 `device_focus`，应回到 Skill 1 补全；L3 `option_a` 亦在 acceptance 对应条目补 `device_focus`。Harness UT PASS 后可写出 `ut/reports/ac-coverage.json`（见正文 SKILL Step 6）。

Skill 6 从 acceptance 过滤 `ut_layer∈{device,both}` 派生 `test-plan.md`。
