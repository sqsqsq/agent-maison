...
> Contract correction (2026-08-30): the earlier explicit-only `code_regression`/coding route is superseded by the testing StepResult contract. An explicit skip without a machine-proven capability fact has no StepResult and remains testing-owned FAIL; this change's delta spec is retained as the formal revision record.
...
- **P0 未豁免 skip 保持 testing 责任（简单优先，fail-closed 不变）**：`p0_coverage_integrity` 保留 BLOCKER FAIL/全分母；gate 分别计算既有 `explicit_skip_tc_ids` 登记、trace 明确 skip、status 为空三组。任何没有 `StepResult` 的 explicit skip/未执行缺口都保持 testing-owned FAIL，零自动 coding candidate；Maison 不从 TC 名称、关联 AC 或报告散文推测原因。只有既有 capability-resolution 以机器事实证明 provider 缺失时，才走 capability/external defer。`code_regression` 与 coding candidate 仅适用于已执行、具备 Hylyre `StepResult` 的 `assertion + assertion_mismatch`，不再绑定 explicit-only。
...
- `harness-gates`：P0 skip 语义（无 StepResult 的 explicit-only 保持 testing-owned FAIL/零 coding；`code_regression` 只表示已执行的 assertion mismatch；`await_human_p0_skip` halt 退役）、P0 优先级对齐锚（`acceptance_to_test_case` 原地断言）、弱化旗标结论规则（`pass_rate_calculated`）、report_validity 职责边界（只约束报告派生候选）。
- `goal-runner`：决策梯删除 P0 专用求人特例；无 StepResult 的 explicit skip 留在 testing，只有可信的 StepResult 机器候选才能走统一 backtrack 路由。

## Impact

- 影响 runtime：`harness/scripts/utils/p0-semantic-gates.ts`（三组分别计算；无 StepResult 的 explicit-only 不写 code_regression）、`harness/scripts/utils/repair-candidates.ts`（仅消费具备冻结 StepResult taxonomy 的 executed assertion mismatch；不注册整个 check）、`harness/scripts/check-testing.ts`（P0 优先级对齐 + 结论=达标规则）、`harness/scripts/goal-runner.ts`（删专用 guard）、`harness/scripts/utils/goal-failure-classifier.ts`（删专用 kind/家族/分类）、`harness/scripts/utils/adjudication.ts`（只读兼容）。
