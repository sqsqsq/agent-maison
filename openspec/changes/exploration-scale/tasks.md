...
- [ ] catalog 卡片可选字段（`NOT_responsible_for`/`easily_confused_with` 降为可选）——**未做**，评估为低风险低优先级打磨项，留后续批次
- [x] small 档红线夹具——`diff_within_scope`/Scope 声明检查代码路径未被 small 档分支触及（regex 精确匹配一次性确认行，不影响 scope_matches_catalog 等其它检查）；未新增独立 fixture 覆盖 small 档端到端场景，是本批已知缺口
...
- [x] `cd harness && npm test` 全绿（缺省 standard + 旧布局零回归）——**1575 单测 + 40 fixtures**（较收口时 1541 净增 34）
