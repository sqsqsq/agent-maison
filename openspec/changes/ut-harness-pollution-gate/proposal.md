...
消费者工程 business-ut 执行 `cd framework/harness && harness-runner` 后，agent 用相对路径 Write UT，产物落在 `framework/harness/{package_path}/...`。现有 `check-ut` 只扫 `<repo-root>/{package_path}/src/ohosTest/...`，误写不可见，仅报「未找到 UT」。
...
- business-ut / harness-cli-cwd / consumer-framework-boundary 文档：Write Path Gate
