...
- [x] ~~ 选定 4 类样本各 ≥1（bugfix / 单模块 / 跨文件中等 / 进行中 NL 修正）；某类不可复现须在报告显式标注缺失~~ —— **[cancelled 2026-07-30]** 用户决定不再执行：评测批次属成本决策（4 类样本 × 2 臂冷启动 ≈ 20 小时机时），非 3.0.0 发布前置；条件与目标明确时另开 plan。
- [x] ~~ 跑批脚本：每臂独立冷启动 headless goal-runner，臂间隔离~~ —— **[cancelled 2026-07-30]** 用户决定不再执行：评测批次属成本决策（4 类样本 × 2 臂冷启动 ≈ 20 小时机时），非 3.0.0 发布前置；条件与目标明确时另开 plan。
- [x] ~~ 指标采集：usage 或代理指标（标注 confidence）/ 轮次 / 门禁命中 / 缺陷数 / 修正样本验证转嫁与否~~ —— **[cancelled 2026-07-30]** 用户决定不再执行：评测批次属成本决策（4 类样本 × 2 臂冷启动 ≈ 20 小时机时），非 3.0.0 发布前置；条件与目标明确时另开 plan。

## 3. 报告与 gate

- [x] 对照报告模板（逐样本 + 口径声明 + gate 建议）：`harness/templates/ab-eval-report.template.md`
- [x] ~~ 产出报告，交用户拍板（继续/收窄）——挪至 Phase 1 完成后宿主工程实测阶段（同上用户决策）~~ —— **[cancelled 2026-07-30]** 用户决定不再执行：评测批次属成本决策（4 类样本 × 2 臂冷启动 ≈ 20 小时机时），非 3.0.0 发布前置；条件与目标明确时另开 plan。
