...
- [x] 4.2b dump 寻址口径统一（2026-08-12 宿主实测新边界，不否定 4.2 的完成态）：
      ...
      命中 :1955「声称已采集但缺失/不可解析——须重采」，把命名问题误报成证据损坏。
...
- [x] 5.4 **SUPERSEDED（非伪报完成）**：verified 机器回执生产已由 `critic-loop-hardening`/delegated provider 落地；独立 critic phase 不再作为本 change 的开放任务，后继 `autonomous-recovery-without-human-gates` 复用 provider 证据并禁止回退到人签。
