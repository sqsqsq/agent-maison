...
- **flow DAG ephemeral by default**: written to the reports/temp location, NOT archived into `{module}/test/dag/`, unless the user explicitly requests archival or a Code Graph `core` node is touched (see `define-code-graph-concepts` / business-ut Step 8.0).
...
- **Phase 5 (business-ut)**. Touched files: `harness/scripts/check-ut.ts` (`loadDagFiles` + coverage checks), `specs/phase-rules/ut-rules.yaml`, `skills/feature/business-ut/SKILL.md` (archival wording), reports path under `doc/features/<feature>/ut/reports/`. Profile overlay `profiles/hmos-app/phase-rules-overlays/ut-rules.overlay.yaml` if hmos-app specifics apply.
...
- **Also ships (merged Track B P3/P4):** module seam/mock registry template + business-ut wiring; characterization path-c (`flow_type`/`origin`, harness gates, `paths/path-c-characterization.md`).
