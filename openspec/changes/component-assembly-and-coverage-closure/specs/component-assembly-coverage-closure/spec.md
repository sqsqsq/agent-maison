...
Completion MUST be verified by the existing workflow/track-derived `verifyFeatureCompletion()` chain. UT/coverage, contract/API, build/runtime, visual/device and manual observations MUST be read from their existing canonical reports, receipts, evidence manifests and verifiers. P3 MUST NOT create a generic evidence registry. An unsupported or unresolvable evidence type remains an explicit gap/blocker.
...
> **Enforced by (P3 implementation):** `harness/scripts/utils/component-closure-evidence.ts`, `harness/scripts/utils/change-unit-feature-projection.ts`, `harness/scripts/utils/verify-feature-completion.ts`, `harness/scripts/check-component-closure.ts`
...
Applicable migration, compatibility, rollback, feature flag, dual-write, controlled fake, temporary adapter/asset, NFR and remaining-risk items MUST derive from current blueprint contract/cross-view/decision/gap facts, CU safe intermediate state/blockers and Feature acceptance/evidence. Each item MUST have an authoritative disposition, owner, needed-by/gate and verification or knowledge-writeback ref. P3 MUST NOT infer PASS from prose or absence. Remaining risk that affects the closure claim requires explicit authorized acceptance.
...
Providers MUST NOT select or omit canonical inputs, bind owners/evidence, delete obligations, set closure verdict, write canonical blueprint/CU/Feature facts, or modify Goal Mode events/receipt/evidence/completion. Provider exit clears transient observations/cache and makes the derived closure stale; formal inputs and accepted evidence remain. No dynamic loader, plugin registry or provider ledger may be added.
