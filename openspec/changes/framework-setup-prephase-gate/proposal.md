...
独立 `/framework-setup` slash 命令与 `00b-framework-setup` 技能跳板易让普通用户误解职责边界；个人 setup 应作为各阶段入口的前置环境检查内联完成，而非单独命令。

## What Changes

- 移除 Claude `/framework-setup` slash 与 `skills-bridge/00b-framework-setup` 对外跳板（保留 `skills/00b-framework-setup/SKILL.md` 内部过程文档）
