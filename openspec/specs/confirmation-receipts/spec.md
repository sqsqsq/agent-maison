...
Define the compatibility boundary for retired quality confirmation receipts. Historical artifacts remain readable for audit and migration, but current quality and recovery decisions are machine-evidence driven.
...
#### Scenario: an external permission remains separate from quality

- **WHEN** execution requires a real secret, irreversible operation, legal approval, or explicit budget authority
- **THEN** the run SHALL use the existing external prerequisite/authorization state, and satisfying it SHALL only enable execution rather than change any quality verdict
