...
- **日期**：2026-06-16
- **验收**：`cd harness && npm test`（864 passed）；`npm run release:verify`（ALL PASS）
- **偏离**：`config-defaults.json` 未 backfill `hmosDevice`（与 plan opt-in 策略一致，仅 template skeleton + field_notes）
