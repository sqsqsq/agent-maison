...
todos:
  ...
  - id: tests-acceptance
    content: >
      ...
      (v1) 含 severity=blocker/major defect 的 pass 屏 → 判不过(pixel_1to1 FAIL)；defects:[]+reverse_missing:[] 且分数达标 → PASS；非法 defect → schema FAIL；**pixel_1to1 下 defects===undefined（finalized）→ BLOCKER/FAIL（与 reverse_missing 对齐、补 [] 解除）、非 pixel_1to1 旧 json 不受影响（D11）**。
...
- **D11 · defects 向后兼容契约（D6 change 敲定）**：pixel_1to1 下 finalized verdict 的 defects===undefined → ratchet **BLOCKER/FAIL** 逼逐屏枚举（补 [] 即解除），与既有 reverse_missing(screensMissingReverseEnum) 对齐；非 pixel_1to1 旧 consumer json 无 defects 不受影响；openspec/MIGRATION 写清此语义（否则省略 defects 字段即可绕过「pass 须枚举为空」）。**F1（GLM review）修正**：此前文档误写 WARN，代码实为 FAIL（反绕过本意）→ 统一为 FAIL + 补 screensMissingDefectsEnum 严重度单测。
