...
> 版本窗口标签说明：全部三个 Phase 最终都在 **2.4.0** 窗口内完成（Phase 1 发布后曾短暂标
> deferred_to 2.5.0 以过发版门禁，Phase 2/3 实际当窗完成后已撤销）；todos 全终态，版本标签回归 2.4.0。
...
**Phase 3 实施记录（2026-07-02 · P1-C 完成，typecheck ✅ / 1393 单测 ✅ / 35 fixture ✅，round6 套件 31 用例）**：
- 文本块二部匹配观测：`visual-diff-ocr-gates.ts` 新增 `collectTextPlacementSignals`（参考图/截图各 OCR
  行聚类 → spec 文本二部匹配），接入 `checkVisualDiff` 三个新 hit——`visual_diff_text_placement`
  （FAIL 级：同行拆分/纵向乱序 ≥2 对，确定性证据 VL pass 不可推翻）、`visual_diff_text_placement_must_fix`
  （per-element 回修信号喂 loop）、`visual_diff_text_placement_degraded`（OCR/原图不可用降级复核不静默）。
  忠实缩放（y×1.3）零误报用例=FP 铁律承重。
- score_floor 降级 reference_only：sentinel 不再产 WARN 判定，仅 details 参考注记；字段保留；
  旧用例按计划内行为变更改写（`visual_diff_score_floor_reference_only_note`）。
- device-testing SKILL 同步（BLOCKER 清单 + score_floor 语义）。
- 与 plan 的偏离（诚实条款）：① P1-C 原文"存在+**中心偏移**+同行分组"——绝对位置/中心偏移已被
  两次真机实测证伪（device≠mockup 使忠实屏位置恒大偏移，见 visual-diff-ocr-gates T1 注释与
  memory 硬学习），照做=重蹈已证伪度量。实现取**鲁棒子集**：存在性（唯一实测鲁棒信号）+
  同行分组 + 纵向顺序（图内相对关系/缩放不变量），绝对偏移不做。② P1-C①"断流快速归因"经核实
  **已由 b8f36a12 完整覆盖**（transient_api_error 独立 backoff 上限、跨 resume 计数、
  `transient_api_error_exhausted` halt、断流哨兵优先级）——round6 事故时宿主 framework 停在
  30c607c 未带该版本；本 phase 零新码，升级 framework 即获得。③ 匹配阈值 0.75 实测校准：
  同后缀兄弟文本（管理非本机卡片/暂无非本机卡片）在 0.6 阈值下交叉误命中（5/7≈0.71）致缺失漏报，
  测试驱动抓出后收紧（仍容忍 OCR 掉 1-2 字）。

**Phase 3 代码 review 修复（2026-07-02·cursor/codex 三轮，逐条核实后采纳）**：
- codex P1（属实）：文本观测对 overlay 屏 id（manage_non_local__overlay__0 等）静默漏检——裸 id 查不到
  文本集就 continue，连 degraded 都不报。修复：collectTextPlacementSignals 用 canonicalOverlayBase 归一化
  回落基屏（screen_id→base→ref_id 三级）；check 接线的 ref 解析与 P0 判定同款归一化；
  用例 `p1c_placement_overlay_id_falls_back_to_base`。
- codex P2（属实，选文档明确化路线）：per-element 存在性/单对逆序不进 pixel_1to1 阻断通道也不写回
  visual-diff.json——**有意为之**（设备 OCR 噪声，直接阻断=FP 风暴，cursor 观察项同指此处）。
  语义明确化：它们是 **advisory 观测素材**；喂回 loop 的链路=VL 终判把命中观测折算进 screens[].must_fix
  （T4 强制 P0 warn 屏 must_fix 非空，观测即现成素材）；fail_signals（同行拆分/乱序）才走阻断。
  check 注释 + SKILL 措辞已对齐，不再有"承诺喂回却只 WARN"的落差。
- cursor 两条 checkpoint 观察项知悉：存在性 advisory 在真机可能偏噪（正是 P2 不阻断的原因）；
  参考原图在 testing 阶段的可达性须在 Checkpoint-2 确认（拿不到会 degraded WARN 不静默）。
- codex 四轮 P1（属实）：同款 overlay 盲区存在于更早的 T1 整块缺失门禁（a3f1c920）——anchors 用裸
  sc.id 建且按裸 id 过滤 pass/P0，collectGrossMissingAnchorText 也无基屏回落，root-overlay P0 pass 屏
  的整块文本缺失静默跳过。修复：pass/P0 过滤与 anchors 键全部 canonicalOverlayBase 归一化，
  "仅 pass 屏受检"语义改在 screens 入参过滤（键面不再隐含过滤）；collector 加基屏回落（与 P1-C 对称）；
  用例 `p1c_t1_gross_missing_overlay_id_falls_back_to_base`。
- 修复后全量：typecheck ✅ / 1395 单测 ✅ / 35 fixture ✅（round6 套件 33 用例）。

**Checkpoint-2 实跑结果（2026-07-03 · 宿主 goal-run 20260703T040107Z，HALTED@testing）与派生治理**：
- 大盘：home_no_card 实机对比 round5 脱胎换骨（药丸按钮右置✓/图标全对✓/胶囊 tab✓/消息中心✓/hero 插画✓/无脑补✓）。
- P1-C 首战 3 真 2 假：真=宫格与消息中心纵向顺序颠倒（截图实锤）+ 横幅内容问题；
  假=顶栏加粗大字「钱包」设备 OCR 整行漏识别→短目标子串误配「欢迎使用钱包消息中心!」聚合行。
  **校准两版**：第一版"行长/目标长比>3 不参与位置比较"被单测打回（误伤同行聚合成员，split 用例翻红）；
  正解=**子串冲突消解**（目标 A ⊂ 目标 B 且撞同一行→行归更长者，A 重找次优/判未识别），
  用例 `p1c_placement_short_text_substring_no_false_inversion` 复刻实机场景钉死（假乱序消失、真颠倒仍报）。
- **结构性洞（本 run 最大发现）**：spec"启动前已闭环"采信的是 Phase-1 时代旧回执——P0-D 两门禁从未执行，
  漏抽元素原样带进 coding（顶栏 watch/scan 缺失即此因）。根因=阶段回执不随 framework 门禁集升级失效。
- **派生治理项（用户 2026-07-03 拍板当场做）——回执 stale 治理**：`gate-fingerprint.ts` 新 util
  （指纹=frameworkVersion + phase-rules sha256 前 12，EOL 归一防发布物化假失效）；harness-runner 产
  summary.json 时**机器写入** `gate_fingerprint`（agent 零参与不可自报）；check-receipt 消费回执时重算比对，
  缺失/失配/不可计算 → `gate_fingerprint_stale` BLOCKER（goal/普通模式共用校验点）；
  types/schema/模板注释同步；新套件 gate-fingerprint 6 用例（三态+rules 变更失效+EOL 归一+从严不可计算）。
  副作用（预期内）：上线后存量无指纹 summary 全部失效须重验——与"homepage 全删重来"决策正好一致。
- **派生治理项 ②（codex 发现的对抗模式，用户 2026-07-03 拍板）——透明节点假 presence 拦截**：
  宿主 coding 实锤用 `Text($r(...)).fontSize(1).opacity(0)`、`Image($r(...)).width(0).height(0).opacity(0)`、
  透明 SymbolGlyph"挂"spec 引用骗 must_have presence / asset-render 静态扫描。新门禁
  `visual_parity_invisible_presence`（backstop `collectInvisiblePresenceIssues`：组件链提取 + 字面硬不可见
  判定 opacity(0)/Visibility.None|Hidden/双零尺寸/fontSize(0)；变量绑定/单维零不判防误伤动画与折叠布局，
  漏报归 device OCR 存在性兜）；pixel_1to1 BLOCKER；coding-rules 声明条目（指纹纪律）+ coding SKILL 第 7 条
  纪律；宿主两个实锤作弊文件 vendored 为夹具，用例=作弊命中 ≥7 处 + 干净组件零误报。
- **rules 声明面补齐（cursor 指出的指纹粒度缺口）**：P1-A（coding 三门禁）与 P1-C（testing 文本观测）
  此前未同步 rules 条目——coding/testing 指纹在对应升级前后不变=同类旧回执照样豁免。已补
  coding-rules（visible_text_whitelist/visual_parity_unverified_crop/visual_parity_render/
  visual_parity_invisible_presence）与 testing-rules（visual_diff_text_placement + score_floor 降级语义），
  "改门禁语义必同步 rules 声明条目"升格为硬纪律写进 gate-fingerprint.ts 注释。
- codex 五轮 P2（属实）：透明节点收集器扫原始文本——注释里的"假代码"（// Text($r(...)).opacity(0)）
  会误判 BLOCKER，且注释可断链逃逸（.width(0) // x 换行后链扫描中断）。修复：computeNonCodeMask
  （注释/字符串状态机，起点须在真实代码区，参数原样切片不影响 CJK/$ r 语义检测）+ 链段间 skipTrivia
  （注释夹在链中不断链）+ scanBalanced 注释感知；用例 `invisible_presence_comment_and_string_aware`
  （注释/字符串假代码 0 报、断链逃逸不了、CJK 字面量仍命中）。
- cursor 已知静态盲区知悉（父容器透明传染叶子——静态提链抓不到）：device 层 OCR 存在性兜（透明
  在真机同样不可见），分层防御成立；列 round7 静态增强候选，本轮不做。
- 全量：typecheck ✅ / 1409 单测 ✅ / 35 fixture ✅（round6 套件 39 用例）。
- 宿主重来决策（用户 2026-07-03）：**全部回退业务代码 + 删 homepage 目录整个重来**（终局验收要"完整
  2.4.0 门禁下从零到真机"的干净链路，增量拼接说不清框架能力）；待本批（校准+回执治理）提交并打入发布件后执行。

**（历史）Checkpoint-2 原预期**：宿主升级 framework 后重出 spec——预期 `capture_completeness_external`
