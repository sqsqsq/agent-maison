...
version: 3.0.1
deferred_to: 3.0.1
# 版本说明：本 plan 立项时当前包版本是 2.4.0，标 version/deferred_to=2.4.1 排入下一窗口；
# 合并主干「轻量化重构」批次（d4a7c1e8）后包版本已跳到 3.0.0（主干 chore(release) 提交
# 30ee7cc），原 2.4.1 标记反而变成"低于当前版本"，会被 check-plan-version.mjs 判定"仍有
# 未完成 todo"FAIL（deferred_to 只在 version > current 时生效，version 变成"过去"后完全
# 不再读取）。此处顺势改为 3.0.1——具体数字（patch/minor）是否合适仍由用户裁定，这里只是
# 保证合并后门禁语义不炸，不代表拍板了正式版本号。
