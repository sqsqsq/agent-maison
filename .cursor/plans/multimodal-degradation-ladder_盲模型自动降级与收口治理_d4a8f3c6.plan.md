...
### 复验 P1 修复批次（2026-07-09，用户拍板直接修不出新 plan）

用户确认三个 P1 改动量小、根因已定位死，直接修不立新 plan（往本 plan 加 pending todo
反而会重新卡住刚通过的 release:check-plans；沿用前两轮 review 修复批次的先例——实施记录
追加于此、提交引用本 plan、不加 todo）。P2×3（盲档 sampled:true 门禁 / refs 锚文本校验 /
金丝雀事件可见性）明确不在本批范围——门禁类需要设计严重度与误报权衡，留待用户决定是否
单独立项。

- **修复①（coding 阶段 OCR 断供假话）**：`resolvePhaseCapabilityAdvisory` 的 ocrJsonPaths
  原来只在 phase==='spec' 时产出；新增 `listExistingOcrPrescanOutputs`（只读列出
  `spec/reports/ocr/*.ocr.json`，不重跑 OCR——spec 仍是唯一生产者），plan/coding 阶段改走
  它。coding prompt 不再对 agent 谎称"no reference images were found"。
- **修复②（plan 阶段旧 pixel_1to1 措辞）**：advisory 的 phase 门从 spec/coding 扩到
  spec/plan/coding——plan phase 现在同样注入能力块，unattended 块的档位措辞分支同源取值，
  不再落回旧 pixel_1to1 人签死路文案。
- **修复③（OCR slug 吞 CJK + 无 source_image）**：`sanitizeOcrPrescanSlug` 字符白名单加入
  CJK 范围（一-鿿），"1-银行卡添卡首页"不再被清成匿名"1-"；`runOcrPrescanForSpec` 落盘的
  ocr.json 顶层新增 `source_image` 字段（project-relative 回指原参考图）——双保险根治
  "盲 agent 靠猜对应哪屏"（ClaudeCode 案 5/7 refs 接线错误的直接诱因）。
- **测试**：+1 新用例（plan/coding 列出已有 ocr.json + prompt 措辞断言）；E6 真实 OCR
  端到端用例改用 CJK 图名并加 slug 保留 + source_image 断言；"非 spec/coding → null"
  既有用例更名为"非 spec/plan/coding（review）→ null"（语义不变）。
- **验收**：typecheck 0 · unit 1545/0（1544→1545）· fixtures 35/0。
- **兼容性说明**：slug 规则变化只影响新产出的 ocr.json 文件名；已有 run 的旧文件名不受
  影响（幂等跳过按新名判断，旧名文件继续可被 listExisting 列出）。
