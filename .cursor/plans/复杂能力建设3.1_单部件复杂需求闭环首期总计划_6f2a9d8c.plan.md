...
# 2026-09-03：3.0.0 已闭环并同步回 main（m0 completed）；e4/b9/d8 由"可顺延 optional provider"
# 改为 3.1.0 必交付（§6.7，作为 m5 前置），依赖 M7 → d8 → m5、M7 → e4 → b9 → m5；三 provider
# 状态由各自 plan 承载并由 release 门独立拦截，本计划不设聚合 todo。
# 2026-09-04：新增 design-entry-convergence 具体待办（§6.8），在 b9 review/提交后、m5 前
# 收敛设计用户入口；不并入 b9 scope、不重开 M7 历史完成记录。
overview: >
...
todos:
  - id: m0-stable-baseline
    content: >
      ...
      本计划不复制其状态（处置口径见 §5.1）。
      **完成记录（2026-09-03）**：用户确认 3.0.0 已在 Br_release_3.0.0 闭环并已完整同步回
      main（cherry-pick -x 逐笔前向传播完成）。仓内佐证：`RELEASE-NOTES-v3.0.0.md` 发布日期
      2026-09-03、发布件口径 `dist/framework-3.0.0.zip`；release 分支头 51a37875；3.0.0 窗口
      16 个在途 change 已全部归档（main 对应 fe49d8d4）；`release:all` 干净树硬闸已落
      （671b8869）。按裁决不重跑 3.0.0 发布；本 todo 据以上发布事实登记完成。
    status: completed
  ...
  - id: m7-formal-requirement-entry
    ...
    status: completed
  - id: design-entry-convergence
    content: >
      部件设计用户入口收敛（2026-09-04 追加，§6.8）：在 b9 开发、review 与提交完成后，
      将 /component-design 作为唯一设计用户入口，撤下 /app-component-blueprint 的公开
      command、Skill 登记与 adapter 跳板；P1 工作流转为内部引用，保留协议、checker、
      调和及三条宿主接缝。创建或继续既有部件设计时，按用户请求与已有产物从未完成步骤
      继续；只读不改设计、重入不自动升 revision，交接所需的首次 CU 分解仍走既有流程，
      不重复创建 CU、不自动施工；同步安装/UPDATE、文档与适用 OpenSpec 的正文及引用，
      保留 e4/b9 消费链。本 todo 独立 review、验证与提交后完成，作为 m5 前置；
      不新建隐藏 Skill 注册机制、状态机或大型子 plan，不修改宿主工程。
    status: completed
  - id: m5-mechanical-loop-gates
    content: >
      framework 最小闭环终验（顺序前置：M0、M6、MG 回归、M7 齐备，且 d8 extension、e4
      conventions、b9 组件资产三份 provider plan 全部 todo 完成——三者状态以各自 plan 为准并由
      check-plan-version release 门独立拦截，本计划不复制，§6.6/§6.7；并完成本计划
      design-entry-convergence 用户入口收敛，§6.8）：
      正常、失败、阻塞、恢复、设计质询和蓝图调和路径均有 unit/fixtures；
...
flowchart TB
    ...
    M7 --> D8["d8 · extension：M7 三接缝的宿主输入通道（3.1.0 必交付）"]
    M7 --> E4["e4 · conventions：蓝图当前事实与设计约束（3.1.0 必交付）"]
    E4 --> B9["b9 · 组件资产：App 蓝图内发现与选型决策（3.1.0 必交付）"]
    D8 --> MG
    B9 --> ENTRY["design-entry-convergence · 用户设计入口收敛（b9 review/提交后）"]
    ENTRY --> MG
    E4 -. real consumption .-> H1A
    B9 -. real consumption .-> H1A
    D8 -. real host input .-> H1A
    C2["c2 · 视觉证据深化"] -. 条件具备才接入 .-> H1C
    B8["b8 · 完整性/授权"] -. execution trust, 非核心前置 .-> MG
```

> 2026-09-03 修订：e4/b9/d8 从"optional provider（可顺延）"改为 **3.1.0 release 前置**，依赖
> M7 → d8 → m5、M7 → e4 → b9 → m5（§6.7；三 provider 状态由各自 plan 承载，release 门逐 plan
> 独立拦截，不设聚合 todo）。"optional"只剩运行时含义——对具体宿主/需求
> 的输入按事实可缺；能力实现不可缺。c2/b8 与 goal 旧基线清理 7b2e4c1a 已于 2026-09-03 顺延
> 3.2.0（各自 plan 已落 version/deferred_to），不在本图关键路径。

> 2026-09-04 追加：b9 review/提交后先完成设计用户入口收敛，再进入 m5（§6.8）；
> 该具体工作由本计划 `design-entry-convergence` 承载，不改变三 provider 各自的状态责任。
...
- **运行时输入 optional ≠ 版本交付 optional**（2026-09-03）：e4/b9/d8 的机制与 M7 消费链
  是 3.1.0 release 前置；某个宿主/需求没有对应输入时只允许形成合法的"未启用/不适用"结论，
  不得把"无输入"写成"能力未实现也可发布"。三者由同一 AI 顺序施工时按 d8 → e4 → b9
  推进（减少同时修改 Skills、adapter 与 prompt 装配面的冲突）；e4 → b9 另有语义顺序
  （b9 的复用纪律消费 conventions）；
- **入口收敛不插入 b9 review**（2026-09-04）：先完成 b9 review 与稳定提交，再独立实施
  `design-entry-convergence`；不得把新增入口要求追认为 b9 缺陷，也不并行改写两者重叠文件。
  m5 验证收敛后的最终发布形态，不先验双入口再撤下旧入口；
...
3.1.0 必须交付 P0–P3、M5A、M6 与 M7（正式需求统一入口与宿主接缝契约），真实宿主准入/
回灌契约，以及 **e4/b9/d8 三个 provider 的框架能力与 M7 消费链**（§6.7；对具体宿主的输入
按事实 optional，能力实现不 optional）。它证明：
...
覆盖；2026-09-03 起再由 §6.7 覆盖：四者齐备之外，d8/e4/b9 三份 provider plan 亦须全部完成**；
M7 自身不执行、不勾选任何 release 门禁项。真实宿主接入（story extension 改造、
内网对接、H1A 语义验收）为发布后的采用工作，按 §8 宿主批次与 M6 回灌契约推进，不阻塞
m5。

## 6.7 e4 / b9 / d8：3.1.0 必交付 provider 与 M7 接线（2026-09-03 追加）

**推进**：G3/G4（e4、b9 进入蓝图设计输入）+ G6（e4/b9 经 CU 施工契约到 review）+ G8
（d8 作为真实宿主输入通道）。

**背景与裁决**：三份 plan 均早于 M7 成稿，把消费点放在 spec/plan/review，并被本计划描述为
"可顺延的 optional provider"。M7 之后这个口径失效：蓝图成为每项正式需求的部件内设计权威，
若惯例、组件资产和上游材料只在 spec/plan 才进入，蓝图就是"只按通用规则设计、不认识本仓
惯例与已有组件、也拿不到真实上游材料"的设计——这正是要用蓝图消灭的缺陷。用户 2026-09-03
裁决：**e4、b9、d8 必须随 3.1.0 发布，不能顺延**；三者在单个需求中仍可"不适用"，但框架能力
必须在 3.1.0 交付——**运行时输入 optional ≠ 版本交付 optional**。不新建第四份统筹 plan；
协议细节由三份既有 plan 各自修订承载（子 plan 内均新增"与 M7 的接线"一节），本节只冻结
依赖、顺序、边界与验收口径。

**依赖与顺序**（§4 图）：M7 → d8 → m5；M7 → e4 → b9 → m5。三 provider 的完成状态由各自 plan
承载并由 check-plan-version release 门逐 plan 独立拦截，本计划不设聚合 todo、不复制状态。d8 与
e4 技术上可并行，
同一 AI 顺序施工时按 d8 → e4 → b9；b9 依赖 e4 的语义顺序是复用纪律消费 conventions。

**三条最小接线**（每条都只复用既有 SSOT、schema、Skill、checker 与 phase，不新增第二真源）：

| plan | 接入位置 | 链路 | 不做 |
|---|---|---|---|
| d8 extension | 宿主 extension skill 与 `mcp_actions.produces` 产出 M7 接缝文件 | 宿主 extension skill → 获取/脱敏上游材料 → 产出 `requirement-source-materialization@1` → 调用 `/component-design` → 消费 blueprint publication 投影 → 必要时提交 `blueprint-review-feedback@1` | 不新增 `before_component_design` 生命周期槽位；`phase_bindings` 仍只管 Feature phases；绑定到 `/component-design` 的知识复用既有单 Skill `skill_assets`/extension skill 路径；manifest 不含 server/URL/token/登录配置；不修改模拟钱包 Story extension，只交付 Maison 协议、入口、示例与交接说明 |
| e4 conventions | `/component-design` → `app-component-blueprint` 的 `current-facts-discovery` 输入 | `conventions.md` → 蓝图当前事实与设计约束（`discovery.facts` 既有 `source_kind: convention` 档 + 决策/节点 provenance 引用）→ CU 施工计划（`contracts.conventions_applied`，e4 t3 交付）→ code review 后置核对（e4 t4 台账） | 不给蓝图新增 conventions 专用字段或第二套引用协议；未启用时 spec/plan/review 行为零变化，蓝图只多一张诚实可用性 Seam Card；只把本次需求真正适用的惯例作为来源、约束或决策依据 |
| b9 组件资产 | App design lens 在 `development`（及涉 UI 的 logical/scenarios）视图 `evolution_impact: changed` 时读取 index/catalog | component-index/catalog → App lens 候选 → 蓝图 `decisions_and_gaps.decisions` 内的扁平 subtype 决策（`kind: component_asset_selection`：target_ref / asset_resolution / component_ref / rationale；provenance 只指 index/catalog 证据来源）→ CU `design_refs` 引用该 decision → Feature plan 投影到 `contracts.components.asset_selection`（b9 t4 交付）→ coding/review 核对 | 不新增 `blueprint.assets`、evidence 状态机、新 phase、第二套 component tree 或 ui-node 映射；无 component-index 时 spec/plan/review 行为不变，蓝图侧 provider 记 unknown/degraded（有 UI 目标）或 not_applicable（本需求无 UI 维度），不伪造选型（运行时未启用 ≠ 可以不交付） |

**既有协议补齐（2026-09-03 codex review 定案，属必要补齐而非额外机制）**：

- provider seam：blueprint-provider-boundary.ts 现只有四个 required provider id、循环也只校验这四个；
  e4/b9 各自在原有 `providers[]` 与**同一** validator/rules 表上增加一张静态 optional Seam Card
  （`conventions-knowledge` / `component-assets`），这是子 plan 的实现 todo，不得再写成"现有清单已经
  支持"；不新增 provider registry、状态或第二套协议；
- 可用性语义：无输入时仍输出该 provider 的诚实可用性结论，不得声称蓝图字节级"行为完全不变"。
  `not_applicable` 只适用于本需求确实没有该维度（e4：明确未启用；b9：本需求无 UI/组件选型维度）；
  配置存在但材料不可读（e4）、有 UI/组件类目标却无 component-index（b9）→ `unknown|degraded`
  （P1 spec「Optional asset provider is missing」既有要求）；b9 的 gap 按既有 admission 二分：当前
  slice 不依赖选型结论 → `open_decision` 可继续，当前 slice 施工依赖选型结论 → `unknown + blocker`
  （needed_by=当前 slice，blueprint-admission.ts 既有规则），不增状态或门禁；
- 选型决策形状：在既有 `$defs.decision` 上定义扁平 subtype `kind: component_asset_selection`
  （`target_ref: view:development/node:<node-id>`、`asset_resolution`、非 custom 必填 `component_ref`、
  adapt/evolve/custom 必填 `rationale`），与既有 `kind: evolution_candidate` 同款先例；
  ...
  decision 一致，`bindings` 才是 Feature 本地信息；关联经既有 CU `design_refs` 与
  `contracts.change_unit.design_ref_mappings`，不新增 decision_ref 协议；
- Story publication：现有 renderer 对 decision 只输出 id/status/owner/verification，宿主 Story 投影
  会看不到惯例约束与选型结论；e4/b9 在同一 renderer 增量输出（采用的 convention id/source_ref；
  component_asset_selection 的 target_ref / asset_resolution / component_ref / rationale），
  仍由同一 `--projection` 校验，不新增 publication schema 或第二份文档。

**验收口径**（写入各子 plan，本节只定边界）：每条链都必须证明**同一事实从蓝图一路传到
CU/plan/review**，而不是只证明文件可读取——e4 证明同一惯例 id 贯穿蓝图 fact/约束 → CU
`conventions_applied` → review 台账；b9 证明"发现已有组件 → 蓝图选型 decision → CU
`design_refs` / Feature `asset_selection` 消费 → review 核对"一条真实机械链；d8 证明 manifest
声明的产物能被 `/component-design` 或三条接缝校验器消费，且 `/extension inspect` 能说明该
消费关系。OpenSpec 先行；完成目标测试后再跑一次与改动风险匹配的全量验证；不修改宿主工程。

## 6.8 M7 后续：部件设计用户入口收敛（2026-09-04 追加）

**背景与裁决**：M7 新增 `/component-design` 后，仍把其内部 P1 步骤
`/app-component-blueprint` 保留为直接用户入口。内部职责分层成立，但用户不应先理解
P1/P2 才能选择设计入口。设计阶段统一呈现 `/component-design`；蓝图仍是唯一 canonical
设计产物，P1 仍负责发现、设计、质询、准入与调和，不反向依赖 CU 数量。
本节覆盖 M7 子 plan t4 的“/app-component-blueprint 保留为直接入口”后续策略；M7 已完成
记录保留，不追溯改写历史，也不重开 P0–P3 的能力架构。

**承载与顺序**：只在本计划 frontmatter 登记 `design-entry-convergence`，不新增大型子 plan
或 provider 聚合项。先完成 b9 开发、review 与稳定提交，再独立实施、review、验证和提交本项，
随后进入 m5。实施前同步适用 OpenSpec 的入口行为；P1 change 仍未归档时原位修订，若届时已
归档则按既有 OpenSpec 流程新增 delta，不修改归档历史。本轮登记不触碰正在 review 的 b9 实现。
规格同步不止修改 `Enforced by`：P1 的 `Static providers consume one provider-neutral protocol`
正文仍要求把 P1 Skill 登记到 index，须改为统一公开入口与 P1 内部流程的关系，并同步实现引用。
d8 现行 `instance-extension-management` 的知识路由要求也提及旧 Skill，相关引用经既有
OpenSpec delta 同步；不回改 archive 历史，不重开 d8 实现。

**实施范围**：

1. 撤下 `/app-component-blueprint` 的公开 command、内置 Skill 索引条目和各 adapter 跳板；
   沿 `skills/reference/*-workflow.md` 既有模式承载 P1 内部流程，由 `/component-design`
   及合法内部消费者引用。同步引用、入口表与计数，不能只删 command 而让 Skill 物化重新暴露它。
2. `/component-design` 负责创建或继续既有部件设计，按用户请求和现有产物从未完成步骤继续，
   不把用户操作枚举为新模式。只读请求不修改 canonical 设计；已有蓝图不等于必须调和，
   仅重入、查看或重跑 checker 不生成新 revision。新事实、权威裁决或冲突解决按既有调和
   规则形成正式修订；意见、建议及未接受反馈不自动升 revision，下游按既有权责重新派生。
   已有草稿未 admitted 时继续设计与质询；已 admitted 但 CU 为 0，且用户要求完成设计交接时，
   调用既有设计准备流程首次生成 CU；已有 CU 复用、不重复创建，变更沿既有修订/superseding
   规则处理。评审复用现有质询与反馈接缝，不建设独立评审流程。用户只要求局部操作时不强制
   跑完分解与交接，不选择 CU 或启动 Goal Mode/P3。
   “局部操作完成”不等于“设计交接完成”；后者仍要求 admitted 蓝图、1..N canonical CU、
   `design_refs` 与 readiness。新建完整设计交接链保持，非正式维护与 App-only 支持边界不变。
3. 保留 P1 协议、schema/checker 接口、三条宿主接缝和 P1/P2/P3 权责边界；完整保留 e4 惯例、
   b9 组件发现/选型及其 CU/Feature/review/Story 投影接线。Story extension 仍接
   `/component-design`，不要求宿主理解内部 P1，不修改任何宿主工程。
4. 同步消费者文档、宿主适配指南和安装/UPDATE 的用户入口呈现；既有物化产物的旧入口收敛
   复用 UPDATE 的 `cleanup-deprecated` 任务及既有 `LEGACY_SKILL_BRIDGE_IDS` /
   adapter `deprecated_artifacts` 声明，按各已物化 adapter 的实际产物补齐明确的旧入口路径，
   先备份再移除，不新增通用孤儿扫描器；仅删除源模板或 index 不会自动清掉旧产物。
   Cursor 的 command 与 Skill 跳板须同时覆盖；generic 使用现有自定义 bundle 路径解析。
   保留用户自有内容，不扩大清理其它入口；用户跳过清理任务时如实说明旧入口仍保留，
   不宣称已清理。现有 extension loader 不按 index 校验
   `skill_assets` 的 skill id，hmos profile 无该旧 Skill 绑定，不为移除登记改造 loader；
   指南将设计入口的知识绑定指向 `/component-design`，仍用既有 `skill_assets` 通道。
   不新增隐藏 Skill 注册机制、可见性开关、别名兼容层、模式字段、状态机或第二套设计流程；
   不扩大为施工与闭环入口合并。

**完成判据**：仓内验证安装/UPDATE 后各 adapter 只呈现统一设计入口、内部流程引用可解析；
新建完整交接与已有设计的局部操作均有对应的流程走查或既有测试覆盖，不能把只改命令名当作
行为已收敛，特别核对无新增事实的重入不升 revision、已 admitted 且 0 CU 可继续完成交接；
e4/b9 的蓝图依据、CU/Feature 消费与评审投影保持连通。review 只覆盖本项及直接
回归；验证按实际改动与仓库要求执行，不重做架构审查或新建演示系统。

具体核对项（复用既有测试，不新增待办或测试体系）：

- `harness/tests/unit/resolve-skill-path.unit.test.ts` 保留 index、三宿主 command、bridge
  三方齐全断言，计数按 b9 提交后的真实基线同步减少该入口（当前 builtin Skill 为 18→17）；
  `harness/tests/unit/component-blueprint.unit.test.ts` 的旧入口可发现性断言改为
  `/component-design` 可解析且 `app-component-blueprint` 不在 index，不削弱其余一致性检查。
- 在 `harness/tests/unit/init-task-executor.unit.test.ts` 的既有 `cleanup-deprecated` 场景中，
  用消费者临时目录放入旧入口并执行 UPDATE 清理，验证旧 command/Skill 跳板移除且备份可恢复、
  `/component-design` 与无关用户内容保留；覆盖 Cursor 两种产物及其它已物化 adapter，含
  generic 自定义 bundle 路径。不能仅凭源模板已删除或 CREATE 无旧入口就宣称 UPDATE 已通过。

**版本收口**：本项是 §6.6/§6.7 所列前置之外的 m5 前置。后续按“b9 收口 → 本项收敛 →
m5 联合终验 → 规格同步/归档与 3.1.0 发布 → 宿主采用与反馈”推进；P1 6.6、P2/P3 7.5、
release-semantics 与发布门禁仍由既有 m5 流程统一收口，本项不提前勾选。m5 复用现有
unit/fixtures 验证 d8 输入、e4 惯例、b9 选型经统一设计入口到 P2 推进与 P3 闭环的接线；
framework 机械验收不冒充真实宿主语义验收，宿主采用继续按 M6 契约回灌。

## 7. 既有 provider 的位置

| 计划/能力 | 本期位置 | 是否阻塞 P0–P3 | 3.1.0 处理原则 |
|---|---|---|---|
| e4 conventions | `/component-design`·P1 蓝图的当前事实/设计约束知识 provider → CU 施工契约 → review 后置核对（§6.7） | 否（P0–P3 已完成） | **3.1.0 release 前置（m5 前置，状态由本 plan 自身承载）**；未启用时 spec/plan/review 行为零变化，蓝图多一张诚实可用性 Seam Card；真实宿主必须实际消费才算对总纲产生语义价值 |
| b9 组件资产 | App 透镜在 UI/development 视图 changed 时的资产发现/选型 provider，决策落蓝图 decision（§6.7） | 否（P0–P3 已完成） | **3.1.0 release 前置（m5 前置，状态由本 plan 自身承载）**；不要求全工程盘点；无 component-index 时 provider 记 unknown/degraded 或 not_applicable，不伪造选型 |
| c2 视觉证据深化 | P3/宿主的验证 provider | 否 | 外部 build/逐例驱动钩子不具备时诚实降级或顺延，不得卡住核心闭环；已于 2026-09-03 顺延 3.2.0 |
| b8 完整性与授权 | 执行可信基础 | 默认否 | 只有出现直接阻断核心证据时才升级前置；不得借机新增第三套账本；已于 2026-09-03 顺延 3.2.0 |
| d8 extension | M7 三条宿主接缝的输入通道 provider（materialization/feedback 产出、publication 消费）与 H1A 宿主输入（§6.7） | 否（P0–P3 已完成） | **3.1.0 release 前置（m5 前置，状态由本 plan 自身承载）**，先于 e4/b9 实施；无 extension 的工程零副作用 |
| 既有 Goal Mode/e5 底座 | 单元执行事实权威 | 是 | 复用，不重建；若 3.0 基线不可信则先收口基线 |

provider 与核心计划可以同窗，3.1.0 的优先级顺序是：**P0–P3 核心闭环 > e4/b9/d8 的 M7 接线 >
其它 provider**。e4/b9/d8 不构成 P0–P3 的能力前置，但自 2026-09-03 起是 **3.1.0 release 前置**
（m5 前置，由 check-plan-version release 门逐 plan 独立拦截），不得再以 `deferred_to` 顺延；c2、b8
与 goal 旧基线清理 7b2e4c1a 已于 2026-09-03 经用户裁决顺延 3.2.0（各自 plan 已落
`version/deferred_to`；7b2e4c1a 的 3.2.0 只是"重新确认兼容窗口是否结束"的最早评估点，不预保证
删除）——不修改发布门禁，不增加豁免。
...
- **设计用户入口收敛完成**（§6.8）：`design-entry-convergence` 完成，用户只通过
  `/component-design` 进入设计活动；原 P1 能力内部复用，完整交接与局部操作边界明确，
  安装/UPDATE、文档与各 adapter 呈现一致，e4/b9 接线不因入口调整丢失；
...
- 宿主演进接缝链至少机械走通一次：实质候选决策卡 → 首次纵切落地 → 替换/缺失/绕过验证
  （M5 fixture）；
- **e4/b9/d8 的框架能力与 M7 消费链已交付**（§6.7；三份 plan 各自 todo 全部完成）：conventions 进入蓝图当前事实与设计
  约束并经 CU `conventions_applied` 传到 review 台账；组件资产在 App 蓝图内完成发现与五级选型
  决策并经 `design_refs` / `contracts.components.asset_selection` 被 CU、Feature 与 review 消费；
  extension 能产出并被 `/component-design` 消费 M7 接缝文件且 `/extension inspect` 可说明该
  消费关系；Story 评审投影能看到采用的惯例与选型结论；三者在没有对应宿主输入时输出诚实
  可用性结论（`not_applicable` 仅限本需求确无该维度，有目标无材料 → `unknown|degraded`），
  **不得把"无输入"写成"能力未实现也可发布"**；
- c2/b8 与 goal 旧基线清理 7b2e4c1a 已顺延 3.2.0（2026-09-03 用户裁决），其未完成项不被误报为
  核心闭环完成，不改门禁、不加豁免；
...
## 15. 实施记录（2026-09-04，m5 联合终验与本地发布收口）

- 本次仅执行 Maison 仓内机制与临时消费者集成验证，未修改真实宿主，未执行 AI 记账、借款、保单或真实设备语义验收；总纲与 G8 真实宿主完成事实不随 m5 升格。
- `cd harness && npm test`：typecheck、unit 4205/4205、fixtures 46/46 通过；MG 17/17、P1 96/96、P2 91/91、P3 40/40。M7 交接 10/10、宿主接缝 18/18、演进工作区 13/13、组件资产 38/38、惯例 35/35、extension 三套件合计 29/29。
- default/release plan 门、独立 `release:verify` 通过后勾选 P1 6.6、P2/P3 7.5，同批更新 release-semantics；P1/P2/P3 按顺序同步并归档至 2026-09-04，活动 change 归零。归档后 OpenSpec strict 37/37、Enforcement 路径校验通过，已归档 e4/b9 的 5 条蓝图 requirement 原样保留。
- 归档使 P1 测试的 active-change 路径失效，实测 95/96；改为确定的归档路径并核对终态后 96/96。未新增解析层或改变运行协议。
- 第一次发布链在空 temp 目录的排除计数断言失败；只修正为实际排除文件数一致性比较，新增一个先红后绿的回归，发布脚本测试 30/30。第二次在消费者 UPDATE 的 docs phase 失败；只补 DOC_INVENTORY 中组件资产文档的必填 role，目标 docs 检查恢复零 BLOCKER/exit 0。两次失败后均暂退 m5 为 in_progress，修复与发布门同批重验，未添加豁免或跳过开关。
- 第三次 `npm run release:all` 在干净提交 `a7a6054bfdab30ad125bfb34a011164c531e38d2` 上完整通过 typecheck、4205 unit、46 fixtures、pack、同 zip verify、同 zip 临时消费者生命周期并生成本地发布件。smoke 9 条中 8 条实测覆盖、1 条已登记退役、零待实现；UPDATE/global/Goal 恢复等后续场景全部完成。
- 产物 `dist/framework-3.1.0.zip`，5,768,094 bytes，1210 个 manifest 文件条目，SHA-256 `26b037bedc87ecc2b82b89461970bbbed487dbb0ea739cae5d8d7a98a866295e`；已核对 sidecar、包内 source_commit 与 staging 清理。m5 据最终结果保留 completed，3.1.0 版本不切换。
- 发布说明、完整命令结果、失败记录与残留项见 [RELEASE-NOTES-v3.1.0.md](../../RELEASE-NOTES-v3.1.0.md)。保留 docs 新鲜度的非阻断提示、Node 弃用告警及 npm audit 超时记录，不宣称依赖安全审计通过。未对外发布、未创建或推送 tag；等待独立 review。
