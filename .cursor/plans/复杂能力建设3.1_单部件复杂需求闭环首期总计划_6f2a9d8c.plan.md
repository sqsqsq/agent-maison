...
todos:
  ...
  - id: m6-real-host-readiness
    content: >
      ...
      UI 单元高保真等按单元提供的最小输入，以及 AI 记账→借款→保单三批验证目标；真实材料
      到位后，由 SE、开发、测试共同评审蓝图的 logical/runtime/development/条件式 deployment 与
      scenarios 以及跨视图关系，真实权责问题按 frontier 成包提交，发现的输入缺口和框架缺口沿
      原链回灌；AI 记账必须真实验证冷启动/恢复、后台自动写入、晚订阅、多页面刷新、分类修改、
      重复事件、进程重建与账号隔离。材料未到位时不造假宿主、不伪造语义 PASS，也不阻塞
      3.1.0 framework 机械能力发布。
    status: pending
isProject: false
---
