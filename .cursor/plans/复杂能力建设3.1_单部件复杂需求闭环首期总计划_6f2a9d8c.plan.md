...
> **Maison 能够面对一个单次 spec/plan 无法安全承载的 App 单部件大型需求，从真实当前态、
> 原始需求和人工提供的已知外部约束，以适配 4+1 形成供 SE、开发、测试共同评审、跨视图一致
> 并主动暴露输入缺口的部件演进蓝图；再分解并连续完成多个有依赖的 Change Unit，最后证明
> 这些单元按共同设计组装成了完整部件能力。**
...
3.1.0 即使全部完成，也只证明“单 App 部件复杂需求核心机制”成立，不宣称长期总纲完成。以下能力
留待后续真实需求驱动：
...
3.1.0 必须交付 P0–P3 以及真实宿主准入/回灌契约。它证明：
