...
状态：**v4 已终审通过，T1–T5 全部实施完毕并吸收一轮 review（2P1+2P2）修订，已提交至 Br_release_3.0.0（265d6929 + f82b159d + 5016acab；未 push、未 cp 主干）**。
**宿主回灌已完成（2026-08-28，用户驱动，framework e21547c8）：验收场景 1/2/8 命中——`navigation.config_files` 经授权后 plan 闭环 PASS 且 `page_registration` 真实 PASS（"全部 3 个 NavDestination 页面已在配置文件中注册"，非 SKIP）；`registration_points` 被拒后删除（角色事实迁 YAML 注释）；`contract_file_reference_closure` 抓到真实声明缺口（20 枚 bank logo 仅被 resource_keys 引用、未入 contracts.files，补授权后 PASS）；`file_completeness` 47 文件 PASS（阶段归属如设计）。全程业务源码/资源/UT 零改动、零提交。只读核验于当日完成。**review 修订（两轮）：①嵌套拒绝扫描删除深度上限（固定层数预算=fail-open，改工作栈迭代+防环）；①b 值侧 containsFileLikeValue 同样改迭代+防环（外层改栈后它是剩下的无防护递归，自引用 YAML 锚点会在装载期爆 RangeError 打断整个 SpecLoader，连结构化 issue 都产不出）；②page_registration 的不可读判定改为就地判普通文件并吞读取异常（原 existsSync 对目录放行，异常逃到 safeRun 会降级 MINOR SKIP 而不阻断）；③openspec 权责改为单向（消费者读取不赋予字段合法性）；④T4 registration_points 负例改为宿主真实层级 navigation.registration_points。验证：`npm test` 3661 unit + 46 fixtures 全绿；`npm run openspec:validate` 42/42；`check-plan-version` PASS。
