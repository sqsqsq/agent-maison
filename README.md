...
**`framework.config.json` 中与 spec 阶段相关的旋钮（opt-in `spec` 段）**：模板默认**不写** `spec` 段；按需手工追加 **`spec.visual_handoff_enforcement`** 等。选型含义见 [skills/project/framework-init/prompts/spec-harness-options.md](skills/project/framework-init/prompts/spec-harness-options.md)；spec 正文写法见 [skills/feature/spec/reference/visual-handoff.md](skills/feature/spec/reference/visual-handoff.md)。
