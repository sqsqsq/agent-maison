...
**Harness 运行时前置**：执行本 Skill 中任意 `harness-runner` / `npx ts-node harness-runner.ts` / `framework/harness/scripts/check-receipt.ts`（依赖 harness npm）前，须满足 [Host harness readiness · Tier_1](../reference/host-harness-readiness.md)。宿主打包/装机/设备工具链仍以本 Skill 的 profile addendum（Tier_2）为 SSOT。
...
4. **与文档门禁的顺序**：本仓储 **`testing` 脚本 harness** 会在校验 Markdown 计划/报告之前尝试 **`device_test.build` → `device_test.install` → `device_test.run`**（profile **SKIP** 则对应步骤 SKIP）。可先撰写文档再由 harness 触发包链路；若 BLOCKER 失败，须先修复宿主 toolchain / 设备再继续闭环。
...
具体派生协议、命令、缓存目录、环境变量等以 profile addendum 为准（见上文 Step 0 所列路径）。
