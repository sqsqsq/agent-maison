...
**Feature 层模块**（如 `<FeatureShell>` / 具体 Feature）——四层均可有内容：
...
| presentation | components/（复杂组件）、pages/（页面；宿主路由容器术语见 profile addendum） |

**BusinessBase / CommonBusiness 层模块**（如 `<BusinessCapabilityModule>`）——侧重 shared + data + domain：

| 内部层 | 典型内容 |
|--------|----------|
| shared | constant/（常量和枚举） |
| data | model/（数据模型） |
| domain | service/（核心服务逻辑） |
| presentation | （通常无 UI，省略） |

**SystemBase / 公共 UI 模块**——侧重 shared + presentation：
...
**设计文档的核心任务之一是将 PRD 中的功能点准确拆分到不同模块**。遵循以下原则（示例模块名仅为占位，**必须**替换为 `doc/architecture.md` 中的真实模块）：

1. **页面和 UI 交互**放入 Feature 层对应模块的 `presentation/`
2. **跟具体业务无关的公共页面**（如首页框架、设置页）放入该 Feature 层的**壳/聚合模块**（示例：`<FeatureShell>`）
3. **某一细分业务域的独立页面和逻辑**放入对应的 Feature 子模块（示例：`<DomainFeatureModule>`）
4. **账号/会话类横切能力**放入 BusinessBase 中对应能力模块（示例：`<AuthServiceModule>`）
5. **与业务无关的基础 UI 组件**（Toast、弹框、基础页面壳子）放入 SystemBase 的**公共 UI 模块**（示例：`<SharedUI>`）
6. **与业务无关的工具能力**（log、格式化工具等）放入 SystemBase 的**公共基础模块**（示例：`<SharedFunc>`）
...
2. **输出功能拆分表**（先于详细设计，供用户确认）：
   ...
   |----------|----------|----------|--------|----------|
   | F1 | xxx | <FeatureModule> | 02-Feature | <说明> |
...
2. **`doc/features/{module}/use-cases.yaml`**（Spec 文件，与 `contracts.yaml` 同目录）：
   ...
   - 规范级样例目录见 `` `profile-skill-asset:5-business-ut/card_opening_dir` ``
...
|-------------|----------|--------|--------|----------|-----------|----------|----------|
| F1 | Tab导航 | P0 | 02-Feature | feature_shell | presentation | HomePage.<ext> | Tabs框架 |
| F7 | 账号状态 | P0 | 04-BusinessBase | auth_module | service | AccountService.<ext> | 登录状态管理 |
...
- Schema：见 `` `profile-skill-asset:5-business-ut/use_cases_schema` ``
- 参考样例：`` `profile-skill-asset:5-business-ut/card_opening_use_cases` ``
- 关键强约束：
...
参考已有示例：`doc/features/home-page/contracts.yaml`（简单 feature，无 use-cases.yaml）、多步流程可参考 `` `profile-skill-asset:5-business-ut/card_opening_use_cases` ``。
