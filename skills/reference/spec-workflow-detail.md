...
"可以偷懒降级"的许可。**交互式能力实测（I1，无 goal-runner 自动注入能力块时）**：不再靠"自己
描述一张图"的软自答（依赖模型诚实、盲模型会幻觉描述）——改走确定性自测卷：按
[interactive-vision-canary](./interactive-vision-canary.md) 后台跑判卷 CLI（随机题卷 + 脚本判卷 +
无感写 `vision.canary` 缓存），判卷结果驱动 `Vision: YES/NO` 分档。**仅当**判卷 CLI 环境不可用/
失败时，才回退到原软自答描述法（用 Read 打开参考图、用自己的话具体描述颜色/文字/布局，给不出
就走盲档）——回退结论**不写缓存**，下次仍尝试自测卷。
