...
0. **设备门由 CLI 自起**（无需 agent 手动前置）：执行 / `--dump-ui-only` / `--observe-ui` 三条碰设备路径启动时，`adhoc-device-test` 在首个设备命令之前自跑入口门并解析注入目标（stderr 锚点 `ADHOC_PHASE=device_gate`）；derive-only 不起门。门失败即不发任何设备命令——读 stderr 的 `reason`（`device_policy_unset` 时是四选一原文，按 [device-policy-gate](device-policy-gate.md) 处置）与 placeholder 的 `error_kind=device_not_ready`；**不得**为此手写解锁脚本或直调 harness 内部函数。
