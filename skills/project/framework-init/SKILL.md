...
- **UPDATE 全 adapter 退役清理**：`cleanup-deprecated` 检查发布件全部 adapter（包括已退出 `materialized_adapters` 的历史残留），备份删除已登记的旧产物与旧 skill/command 跳板（如 `prd-design`、`requirement-design` 及已撤下的 `app-component-blueprint` 设计入口，覆盖 Cursor command/Skill、Claude/Codeagent command、Codex/Chrys/OpenCode Skill 与 generic 配置 bundle 的 Skill 跳板），并按 `hook_configs` 移除退役脚本注册；保留 `component-design`、现行扁平名（`spec`、`plan`、`coding` 等）、现行产物与宿主其他配置。若 S3 执行了该任务，摘要须引用 run-log 中 `cleanup_effects.backup_deleted` 与 `hook_configs_updated` 计数；有备份时注明 `.framework-backup/<timestamp>/` 路径（S1 probe 仅提示「将清理」，以 S3 run-log 为准）。**跳过**该任务则旧入口仍留在实例根，必须如实披露，不能宣称已收敛；"无 deprecated"不等于磁盘无任何未登记残留。
