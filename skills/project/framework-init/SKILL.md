...
- **UPDATE 全 adapter 退役清理**：`cleanup-deprecated` 检查发布件全部 adapter（包括已退出 `materialized_adapters` 的历史残留），备份删除已登记的旧产物与旧 skill/command 跳板，并按 `hook_configs` 移除退役脚本注册，保留现行产物和宿主其他配置。摘要须引用 run-log 中 `cleanup_effects.backup_deleted` 与 `hook_configs_updated` 计数；有备份时注明 `.framework-backup/<timestamp>/` 路径（S1 probe 仅提示「将清理」，以 S3 run-log 为准）。**跳过**该任务则不清理；“无 deprecated”不等于磁盘无任何未登记残留。
