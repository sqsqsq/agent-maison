...
- **怎样做**：S2 选 `init.architecture_preset=preset_5_layer`；将 `` `profile-skill-asset:framework-init/preset_5_layer_sample` `` 解析得到的 `architecture` 合并进 `configWritePayload`（并补全 `project_name`、`paths` 等模板字段）。
