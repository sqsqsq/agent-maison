...
4. 有人在场且需求 UI 相关的新 run，必须先按 [interactive-vision-canary.md](../../reference/interactive-vision-canary.md) 完成 interactive canary（新鲜缓存会自动 `SKIP`），再消费同一条 effective image-input 结论：primary blind 时才按 personal setup S2.1 选择合法 provider 或明确「跳过并盲跑」。随后才用 operations 文档的 `goal-mode-entry.ts --prepare-run --run-mode attended --feature ... --requirement ... --adapter ...` 创建 manifest/run-control；跳过选择必须在这次 prepare 加 `--allow-blind-visual`。未授权 BLOCKER 不得留下 manifest/run-control，补好 provider 或旗标后重新 prepare。已有 run 只按同一 `run_id` 恢复。attach 同样必须显式传 `--run-mode attended`，且 `--adapter` 必须等于 manifest adapter；attach 携带 `--allow-blind-visual` 必须明确拒绝，host bridge 在取得 run-control owner/epoch 前只保留防御性校验，不承担首次裁决。无人值守只走 `goal-runner --detach`。
