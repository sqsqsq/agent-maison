...
| `typical_business_terms` | 业务团队/PM 在口头 / spec 里称呼这个模块时用的词（反向索引） |
...
- 架构级变更记录表的准入条件和触发时机以 [plan · Step 12](../../feature/plan/SKILL.md) 为准，事件描述格式：`| YYYY-MM-DD | module_set_change \| responsibility_rewrite | <具体变化> |`。
- 若 catalog 变化是由一个具体 feature 的 design 驱动的，且该 design 已经声明了 `architecture_impact != none`，则在 plan · Step 12 分支中统一处理，不要在本 Skill 里重复追加。
...
#   1) 翻你过去 3 个月的 spec / 需求评审纪要，把里面出现的业务名词摘下来
...
- **Traceability**：`easily_confused_with.module` 指向的模块必须在本文件内存在；`entry_file` 路径在磁盘上真实存在（WARN）；`key_exports` 与该模块 profile 导出入口的 top-level export 保持同步（漂移即 WARN）；**feature 反向完整性**（`feature_scope_integrity` WARN）：扫描各 feature 的 `spec/spec.md` 与 `plan/plan.md`（兼容旧扁平路径）的 Scope 声明，提示哪些 feature 引用了本 catalog 未建档的模块——这些 feature 在跑 `--phase spec/plan`（legacy prd/design 仍 alias） 时会 BLOCKER on `scope_matches_catalog`，catalog 阶段提前告警。
