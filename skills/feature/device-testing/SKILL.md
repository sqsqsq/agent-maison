...
9. **Step 5 生成测试报告**（`testing` harness PASS 且 trace.json 已写出后）：模板 `templates/test-report-template.md`；Step 5.1 自 trace 回填执行状态（详见 reference）；填充测试概览/执行结果/缺陷清单/通过率统计/结论 5 章节；结论判定：P0=100%且总体≥阈值→达标，P0=100%但总体<阈值→有条件达标，P0<100%→不达标。**逐信号复核块（adjudicated-repair-loop，存在 actionable 视觉信号时必填；否则停等求人）**：每条结构化视觉信号在报告末尾 fenced `defect-review` 块逐条复核（`signal` 精确填 defect 稳定指纹 `screen|class|element|bbox_bucket`、`verdict: confirmed|disputed`、`rationale` 理由）——confirmed（同向）才会物化为可回退候选；disputed / 未复核 / producer 归为 uncertain 一律停等，不产生自动回退。**本块无人工终裁效力**：人工恢复走既有 visual-confirm 通道（visual-diff.json 屏条目 `confirmed_by` 真人人签；isHumanVerified 谓词校验，非自动化身份），人签=人工确认该屏视觉无需修复；确要修复则改 verdict=fail + must_fix 走既有 actionable 通道。即席模式无强求写 test-report.md。
