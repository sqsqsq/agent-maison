...
9. **Step 5 生成测试报告**（`testing` harness PASS 且 trace.json 已写出后）：模板 `templates/test-report-template.md`；Step 5.1 自 trace 回填执行状态（详见 reference）；填充测试概览/执行结果/缺陷清单/通过率统计/结论 5 章节；结论判定：P0=100%且总体≥阈值→达标，P0=100%但总体<阈值→有条件达标，P0<100%→不达标。**逐信号复核块（adjudicated-repair-loop，存在 actionable 视觉信号时必填；否则停等求人）**：每条结构化视觉信号在报告末尾 fenced `defect-review` 块逐条复核（`signal` 精确填 defect 稳定指纹 `screen|class|element|bbox_bucket`、`verdict: confirmed|disputed`、`rationale` 理由）——confirmed（同向）才会物化为可回退候选；disputed / 未复核 / producer 归为 uncertain 一律停等，不产生自动回退。**本块无人工终裁效力**：人工判为误报或已解决时，由 runner 外签发并注入可信 `human_visual_acceptance` receipt 后再 resume；确认为真缺陷时，写 `verdict=fail` + `must_fix` 后再 resume。`confirmed_by` 在本恢复路径仅作展示，不构成授权。即席模式无强求写 test-report.md。
