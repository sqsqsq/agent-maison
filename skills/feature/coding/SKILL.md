...
[ ] 13. 视觉背板（ui_change=new_or_changed）：`pixel_1to1` 下带 `color_ref`/`semantic_role` 的 ui-spec 节点是否已在 **visual-parity.yaml 映射到 contract_component**，且该 struct 源码引用对应 `$r('app.color.*')`？`must_have_elements`（搜索框/角标等）是否在组件树或 string/源码命中？占位品牌资产是否已显式 placeholder + 用户知情？
```
