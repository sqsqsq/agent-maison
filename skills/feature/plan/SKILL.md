...
> **为什么这一步如此重要**：`contracts.yaml` 是 coding 阶段 编码时的强契约——文件路径、接口签名、组件 Props 必须与 contracts.yaml 一致；`use-cases.yaml`（若存在）是 Skill 5 业务级 UT 的蓝图——DAG 与 UT 用例按 branches 1:1 生成。Harness 也依赖它们做接口一致性验证与 UT 覆盖追溯。未达阈值的简单 feature 不产出 `use-cases.yaml`，避免为测试工具人为引入架构复杂度。
...
- 更新 [doc/module-catalog.yaml](../../../../../doc/module-catalog.yaml)：新增 / 删除 / 迁层对应模块条目，见 [Skill 0 catalog-bootstrap](../../project/catalog-bootstrap/SKILL.md) Phase A 的增量流程。
...
| **Skill 4 (Code Review)** | plan.md + contracts.yaml | 对照检查实现一致性 |
| **Skill 5 (业务级 UT)** | plan.md + contracts.yaml | 读取业务流程信息生成 DAG |
