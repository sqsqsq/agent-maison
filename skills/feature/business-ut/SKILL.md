...
- 若目录存在但本阶段输入缺失（至少 `spec.md`、`plan.md`、`contracts.yaml`、`acceptance.yaml`）：报告缺失文件并回到上游阶段补齐；不得把同名归档当作上游产物。
- 继续执行前，向用户展示本阶段输入矩阵：`spec.md` / `plan.md` / `contracts.yaml` / `acceptance.yaml` / `use-cases.yaml(可选)` 存在/缺失，旁证归档/同名前缀条目如实列出但明确忽略。
...
你的任务是作为**既有代码的消费者**：读懂业务编排源码（由 coding 自选形态……

本 Skill 是项目全生命周期流水线的**第五环**。上游输入来自 plan 阶段（`use-cases.yaml`，**条件式**）、coding（业务编排源代码 + UI）与 code-review（Code Review），输出（UT + DAG + 可选 `ut/reports/ac-coverage.json`）将流入 device-testing（真机测试；消费 `acceptance.yaml` > `device_focus`）。
...
- 🔴 **不要为了 UT 反过来改架构**：不要求新建特定目录或接口形态；若代码可测性差（如业务嵌在 inline lambda 中），反馈 coding 抽出命名方法，不要在 UT 里实例化 UI 组件。

### v2 → v2.1 的关键澄清

| 维度 | v2 老表述 | v2.1 新表述 |
|------|-----------|-------------|
| 被测单元 | **UseCase 类**（必须在 domain/usecase/） | **命名业务入口**（Page 方法 / 普通 Flow 类 / 导出函数，由 coding 自选） |
...
| UI 交互 | 部分在 UT 里走 | ✗ 交 device-testing（`acceptance.yaml` > `device_focus`） |
| AC 过滤 | 全部算 UT 覆盖 | `ut_layer in [unit, both]` 进 UT；`device` 交 device-testing |
...
- **失败归因**：若 `check-ut` 报命令形态不匹配，应优先核对 profile provider 命令形态，**不要**未经对齐就按依赖缺失处理或进入 device-testing。

## 输入

| 输入项 | 必需 | 说明 |
|--------|------|------|
| **`<features_dir>/{feature}/use-cases.yaml`** | ⚠️（仅复杂 feature） | plan 阶段 产出（仅当满足复杂度阈值）；含 `coordinator / ui_bindings / data_boundaries / state_model / branches`，business-ut 的**主规划来源** |
| 业务编排源代码 | ✅ | coding 产出；代码形态由 coding 自选（Page 命名方法 / `Flow` 类 / 导出函数）。UT 按 `ui_bindings.user_actions.calls` 或 acceptance.yaml 指向的函数直接调用 |
| data 层源代码 | ✅ | `data/repository/*.<ext>`、`shared/client/*.<ext>` 等；UT 在 profile 允许的边界上打桩 |
| `<features_dir>/{feature}/contracts.yaml` | ✅ | 接口契约 Spec，`data_boundaries[].type` 必须来自这里的 `interfaces[].class` |
| `<features_dir>/{feature}/acceptance.yaml` | ✅ | 验收标准 Spec，含 `ut_layer`；简单 feature 时是主规划来源 |
| `<features_dir>/{feature}/ut/testability-audit.md` | ✅ | Step 1.5 可测性预检（覆盖全部 unit/both AC/BD） |
| `<features_dir>/{feature}/ut/mock-plan.yaml` | ⚠️ | Step 1.6 Test Double Plan；存在 L0/L1/L2 可测项时 **必填** |
| `<features_dir>/{feature}/plan/plan.md` | ✅ | 状态机 Mermaid、UseCase 清单章节（若有） |
| `<features_dir>/{feature}/spec/spec.md` | ✅ | 业务流程图和异常场景 |
| `doc/architecture.md` | ✅ | 模块架构全貌 |
| `review-report.md` | ❌ | 可选，用于确认代码已通过 Review |

**若缺少 `use-cases.yaml`**：不阻塞本 Skill。按 acceptance.yaml + dag.yaml 直接针对 data 层 / 导出函数写 UT；harness 会以 WARN 提示而非 BLOCKER。**严禁**为此回过头去要求 plan 阶段 补 use-cases.yaml 以套入架构（除非确实符合复杂度阈值）。

**若缺少 `acceptance.yaml`**：提示用户先运行 spec 阶段。
...
| UT 模板 + Spy 模板（子类化既有类 / 原型替换） | `` `profile-skill-asset:business-ut/ut_template` `` |
...
以下结论按本仓库计划书落地，作为 business-ut 的 **SSOT** 口径：

1. **存量 feature 迁移**：已在历史版本通过 UT harness 的 feature，**仅当再次进入 business-ut 并变更 UT 相关产物时** 回补 `ut/testability-audit.md` 与 `ut/mock-plan.yaml`；**新 feature 自 v2.3 规则生效起一律强制**（与 `ut-rules.yaml` 中 BLOCKER 一致）。
2. **L3 + option_b 接缝白名单**：仅允许 **构造注入、包装 wrapper、提取命名方法、setter 注入** 等显式接缝；**禁止**「换一种全局单例」式改造敷衍 UT。
3. **可测性预检的独立切入**：如只想完成 Step 1.5/1.6（产出 `testability-audit.md` / `mock-plan.yaml`）后暂停，请在 `/business-ut`（Cursor 即 `business-ut` 跳板）入口中明确告知 agent，**不**再提供独立 `/ut-audit` slash 或跳板；完整 UT 闭环仍由 `/business-ut` 收尾。
...
1. 可选产出辅助文档 `<features_dir>/{feature}/ut/quick-plan.yaml`（模板：`` `profile-skill-asset:business-ut/quick_plan_template` ``）
...
在输出下文 **「UT 规划清单」** 之前，必须完成本 Step 并落盘 **`<features_dir>/<feature>/ut/context-exploration.md`**（**`schema_version: "1.1.0"`**）。

**上下文摘取（BLOCKER）**：禁止通读大模块源文件。按 `` `profile-skill-asset:business-ut/context_extraction_protocol` `` 执行 rg 签名摘取；总上下文 **≤ 300 行**。`source_code_paths` 只列被测入口与 UT 目标，不列整模块目录。
...
- `<features_dir>/{feature}/use-cases.yaml`（**若存在**）
- `<features_dir>/{feature}/acceptance.yaml`（只关注 `ut_layer in [unit, both]` 的 AC/BD）
- `<features_dir>/{feature}/spec/spec.md` / `plan/plan.md`
- `<features_dir>/{feature}/contracts.yaml`（data_boundary type 必须在 `interfaces[].class` 中）
- 业务编排源代码（coding 自选了 Page 方法 / `Flow` 类 / 导出函数）
...
> - 本轮覆盖的 `AC/BD/branch`，以及不覆盖项和原因（如 `device` 交 device-testing）；
...
- [ ] 写完后运行（`<path>` 用相对工程根路径，如 `<features_dir>/{feature}/ut/testability-audit.md`）：
  `cd framework/harness && npm run validate:ut-artifact -- --type testability-audit --file <features_dir>/{feature}/ut/testability-audit.md`

在生成 DAG / mock-plan / **profile 定义的单测源文件**之前，必须为 **acceptance.yaml 内每条 `ut_layer ∈ {unit, both}` 的 AC/BD** 写一条可测性结论，归档：

`<features_dir>/{feature}/ut/testability-audit.md`

1. **按模板撰写**：`` `profile-skill-asset:business-ut/testability_audit_template` ``（L0–L3 定义、依赖 kind/seam、YAML 形态）。
2. **对每条 unit/both 项给出**：
...
- [ ] 写完后运行（`<path>` 用相对工程根路径，如 `<features_dir>/{feature}/ut/mock-plan.yaml`）：
  `cd framework/harness && npm run validate:ut-artifact -- --type mock-plan --file <features_dir>/{feature}/ut/mock-plan.yaml`

在 Step 2 之前产出类型骨架，路径：

`<features_dir>/{feature}/ut/mock-plan.yaml`

1. **规格**：`` `profile-skill-asset:business-ut/mock_plan_schema` ``（imports、`spies[]` 或 `doubles[]`、每条 `strategy: spy | mockkit | fake | prototype_patch`、methods、presets；`ts_expr` **必须**含 `as Type` 或 `new ...(`）。
...
- `<features_dir>/{feature}/ut/reports/flow-dag/{flow_id}.dag.yaml`

**显式归档**（可选）才写入：

- `{module}/test/dag/{flow_id}.dag.yaml`

完成 UT 阶段前，当存在 **ut_layer ∈ {unit, both} 且 priority ∈ {P0, P1}** 的 AC/BD 时，须由 business-ut 产出机器可读 **`<features_dir>/{feature}/ut/reports/coverage-evidence.json`**，且 **`mappings[]` 须覆盖每条上述 P0/P1 scope**（见 `` `profile-skill-asset:business-ut/coverage_evidence_schema` ``）。harness 只校验、不自造 mapping。
...
3. **UI 副作用不进 UT 断言**：`NavPathStack.push` / `showToast` 只能作为 `ui_subscription` 节点记录，或在 spec 阶段 的 `acceptance.yaml` > `device_focus` 中声明，不要画成 `port_call_*` 或 `assertion` 节点
...
6. **写入** 默认 `<features_dir>/{feature}/ut/reports/flow-dag/{flow_id}.dag.yaml`（仅用户要求归档或触及 core 节点时写 `{module}/test/dag/`）
...
按照 `` `profile-skill-asset:business-ut/ut_template` `` 提供的骨架生成。**直接调用 `ui_bindings.user_actions.calls` 声明的命名函数**，**不 new `@Component struct`**：
...
[ ] 12. device 层 AC：spec 阶段 的 acceptance.yaml 已为 ut_layer∈{device,both} 填写 device_focus；DAG 的 ui_subscription 要点与 device_focus 一致
...
**不再**产出 `device-testing-todo.md`（已废弃）。真机要点由 spec 阶段 写入 `acceptance.yaml` > `device_focus`。本步在 harness PASS 后由脚本写出：

`<features_dir>/{feature}/ut/reports/ac-coverage.json`（unit 层覆盖摘要，**非** acceptance SSOT）。

若发现 device/both AC 缺 `device_focus`，应回到 spec 阶段 补全，而非新建平行 todo 文件。
...
| 交 device-testing 的 device AC | K 条（见 acceptance.yaml device_focus） |

### 下一步
- 运行 Harness 验证（Step 8）
- 四件套 PASS 后 **`ut.ok_to_testing` / `phase.next_step` 停等**（user-confirmation-ux §8）；真机 test-plan 须在用户授权 device-testing 后再派生
```

### Step 7.5：UT 编译闭环（必要出口）

> v2.2 新增：**UT 编译/宿主静态检查是 business-ut 的必要出口条件**。光"写完"不算，必须让当前 profile 的 `ut.compile` capability 实际通过；本步骤要求 agent 自己跑闭环。
...
> **不要**让 agent 自己手敲宿主 UT 编译命令。目标 task / 模块定位 / env 注入由 harness 与 profile provider 封装，日志会落到 `<features_dir>/<feature>/ut/reports/` 便于排错。

#### 7.5.3 自闭环修复策略

1. `ut.compile` 对应规则 FAIL → 进入修复；
2. 完整 Read `<features_dir>/<feature>/ut/reports/` 下的失败日志；
3. 按错误类型分类：
   - UT 调用的被测函数签名不符 → 修 UT；
   - UT import 路径错误 → 修 UT；
   - 类型注解与被测实际类型不匹配 → 修 UT；
...
1. **立即停手**，向用户输出请求：
   ...
   - 影响面（会触发 coding 的哪些 BLOCKER 重跑）。
2. 用户**书面同意**前不得修改任何源码文件；
3. 同意后把授权登记到 `<features_dir>/<feature>/ut/reports/gap-notes.md > approved_src_mutations[]`（含时间戳、文件、变更摘要、用户原话）；
...
一次 `--phase ut` 可同时触发 `ut.compile` + `ut.run`；日志与 summary 落到 `<features_dir>/<feature>/ut/reports/`，运行报告 details 会包含 provider 声明的失败阶段标签，方便定位。
...
2. failed > 0：
   - 读 `<features_dir>/<feature>/ut/reports/hdc-test.log` 完整内容；
...
读取 `<features_dir>/<feature>/ut/reports/ut-install-diag.json`（harness 装机前写入）与 `ut_hvigor_test` 报告：
...
优先读取 `<features_dir>/<feature>/ut/reports/summary.json`，禁止用 `grep` 解析完整控制台日志。
...
- `<features_dir>/{feature}/use-cases.yaml`（v2 新增）
- `<features_dir>/{feature}/contracts.yaml`
- `<features_dir>/{feature}/acceptance.yaml`
...
- **v2.1 语义检查**：
  ...
  4. `branch_coverage_semantic` — branches 是否涵盖 spec 中所有异常路径（若有 use-cases.yaml）
...
1. `<features_dir>/<feature>/ut/reports/trace.json` 真实存在；
2. 脚本 harness 退出码 0、零 BLOCKER；
3. verifier 子 agent 报告 verdict = PASS；
4. 完成回执 `<features_dir>/<feature>/ut/phase-completion-receipt.md` 已填写并通过 `cd framework/harness && npx ts-node scripts/check-receipt.ts --feature <feature> --phase ut` 校验。
...
四项全部通过后，业务级 UT 阶段完成，**具备**进入 device-testing（真机测试）的**资格**；**不授权**自动开 device-testing。
...
- 上游输入:
  - `<features_dir>/{feature}/use-cases.yaml`（plan 阶段 v2.1 产出，仅复杂 feature）
  - 业务编排源代码（coding v2.1 产出，代码形态由 coding 自选：Page 命名方法 / `Flow`/`Coordinator` 普通类 / 导出函数，**不强制** `domain/usecase/` 目录）
  - data 层源代码（`data/repository/*.<ext>` / `shared/client/*.<ext>` 等）
  - `<features_dir>/{feature}/plan/plan.md` / `spec/spec.md` / `contracts.yaml` / `acceptance.yaml`
...
| 消费者 | 消费的产出 | 用途 |
|--------|-----------|------|
| **device-testing (真机测试)** | `acceptance.yaml`（device_focus）+ UT + DAG | 真机 test-plan 与追溯 |
| **Harness (验证层)** | use-cases.yaml + DAG + UT | 脚本/AI 验证 UT 质量 |
| **开发者** | DAG + 业务编排源码 | 理解业务流程，维护 UT |

## 约束与注意事项

1. **UT 是消费者，不驱动架构**：**绝对禁止**为了 UT 反向要求 plan 阶段/3 新增特定目录下的 `XxxUseCase` 类或 Port 接口。若业务嵌在 `onClick = () => {}` 内，应反馈 coding 抽出命名方法，而不是在 UT 里实例化 **UI 组件**。
...
12. **【HARD STOP — 不可绕过】禁止擅自修改业务源码**：business-ut 阶段 agent 对 **受保护业务源码前缀**（定义见 `check-ut.ts` 与 profile，不再写死 `02-Feature` 等目录名）下、且**非 profile 声明的测试/夹具源目录**内任何文件的修改，**必须**满足以下全部条件：
    ...
    1. **动手前**显式向用户提出请求（`ut.src_mutation` · freeform + portable；**须先展示完整变更描述**）：
       ...
       - 预估影响面（会触发 coding harness 的哪些规则重跑）。
    2. 用户**书面同意**后方可动手（对话中明确 "同意" / "approved" / "OK" 等正面表述）；
    3. 动手后必须把授权纪要写入 `<features_dir>/<feature>/ut/reports/<timestamp>/<model>-ut/gap-notes.md > approved_src_mutations[]`：包含时间戳、文件路径、变更摘要、用户确认原话/链接。
    ...
    违反 HARD STOP 的行为会被后续 code-review（Code Review）追溯并标记为质量事件。
...
- **路径约定**：`<features_dir>/<feature>/ut/reports/<timestamp>/<model>-ut/trace.json`
