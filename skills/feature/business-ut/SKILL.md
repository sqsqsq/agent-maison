...
| 输入项 | 必需 | 说明 |
|--------|------|------|
| **`doc/features/{feature}/use-cases.yaml`** | ⚠️（仅复杂 feature） | plan 阶段 产出（仅当满足复杂度阈值）；含 `coordinator / ui_bindings / data_boundaries / state_model / branches`，business-ut 的**主规划来源** |
...
| `doc/features/{feature}/plan/plan.md` | ✅ | 状态机 Mermaid、UseCase 清单章节（若有） |
| `doc/features/{feature}/spec/spec.md` | ✅ | 业务流程图和异常场景 |
...
1. 可选产出辅助文档 `doc/features/{feature}/ut/quick-plan.yaml`（模板：`` `profile-skill-asset:business-ut/quick_plan_template` ``）
...
- `doc/features/{feature}/spec/spec.md` / `plan/plan.md`
...
完成 UT 阶段前，当存在 **ut_layer ∈ {unit, both} 且 priority ∈ {P0, P1}** 的 AC/BD 时，须由 business-ut 产出机器可读 **`doc/features/{feature}/ut/reports/coverage-evidence.json`**，且 **`mappings[]` 须覆盖每条上述 P0/P1 scope**（见 `` `profile-skill-asset:business-ut/coverage_evidence_schema` ``）。harness 只校验、不自造 mapping。
...
- 上游输入:
  - `doc/features/{feature}/use-cases.yaml`（plan 阶段 v2.1 产出，仅复杂 feature）
  ...
  - `doc/features/{feature}/plan/plan.md` / `spec/spec.md` / `contracts.yaml` / `acceptance.yaml`
