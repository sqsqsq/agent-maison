...
> 下面这段应出现在 `doc/features/card-opening/design.md` 的 **"六、业务流程 UseCase 清单"** 章节。本 feature 满足 Skill 2 Step 6.1 的复杂度阈值（多 UI 节点共享状态 + 多步云调用 + 含回滚分支），因此产出 `use-cases.yaml`。完整 Schema 见 `framework/skills/5-business-ut/templates/use-cases-schema.md`。
