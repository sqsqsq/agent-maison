...
所有 Skill 均为 Markdown 正文；各 **agent adapter** 决定如何在实例根暴露入口（slash、技能跳板、仅全局说明文件等）。执行某阶段前请**完整阅读**对应 `framework/skills/<scope>/<skill-id>/SKILL.md` 及其引用的 template / reference。

机器可读索引 SSOT：[`skills.index.yaml`](skills.index.yaml)（由 `resolveSkillPath` 消费）。

## Skill 跳板与「单点真相」

当某 adapter 在实例根生成了指向本目录的轻量入口文件时，该文件**只作跳转**：完整规则、BLOCKER、harness 命令与 verifier 要求**一律**以 `framework/skills/<scope>/<skill-id>/SKILL.md`（及同目录 `prompts/`、`templates/`、`reference/`）为准。
...
**[`framework-init`](project/framework-init/SKILL.md)** 是其它 Maison Skill 的项目就绪前置：实例根须先有有效的 `framework.config.json` 以及初始化约定的目录与入口文件（路径以配置中 `paths` 为准）。无竞争主动作的显式选择/调用 framework-init，或明确接入发布件、创建/迁移 config、刷新 adapters 才进入该 Skill；Skill 名称/链接被动出现不单独触发。普通 Git/SCM L0 请求命中时只退出 init 子流程，不结束本轮，须继续完成最新获授权主动作。
...
**Harness 运行时前置（Tier_1）**：在 feature phase skill 中执行任何 `harness-runner` / `npx ts-node harness-runner.ts`、或在缺少 harness npm 依赖的前提下执行 `framework/harness/scripts/check-receipt.ts` 等脚本之前，须先满足 **[`reference/host-harness-readiness.md`](reference/host-harness-readiness.md)**（与 framework-init **S3 `harness-install`** 中 Tier_1 等价；init 流程仍须在 S3 **落地执行** `npm install`，而非仅阅读本文）。
...
```text
framework/profiles/<project_profile.name>/skills/<skill-id>/profile-addendum.md
```
...
| Skill id | 路径 | 摘要 |
|----------|------|------|
| framework-init | [project/framework-init/SKILL.md](project/framework-init/SKILL.md) | 显式选择/调用或明确接入/刷新；Git/SCM 退出 init 后继续 L0 主动作 |
...
| 60 | device-testing | [feature/device-testing/SKILL.md](feature/device-testing/SKILL.md) | 测试计划与报告 |

如需仅做可测性预检与 mock-plan（business-ut 的 Step 1.5/1.6），仍在 **`/business-ut`（Claude Code）或 `business-ut` 跳板（Cursor）** 入口启动，并在用户消息中明确「仅做到 Step 1.5/1.6 后暂停」；SSOT 仍为 business-ut 正文。
