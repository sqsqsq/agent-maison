...
| `prd.visual_handoff_enforcement`（v2.9+） | 同上 | **PRD 脚本**中 Visual Handoff 守门强度；决策表见 [prompts/prd-harness-options.md](prompts/prd-harness-options.md)。**CREATE 默认**：与模板一致，取 **`warn`**（利旧 PRD，未写 `ui_change` 仅告警不失败）。若用户希望强约束，在 Step 2 **显式追问一次**并记录选择 `strict` / `off`。**UPDATE**：老 config 已有 `prd` → **原样保留**；没有 `prd` → 建议追加 `{ "visual_handoff_enforcement": "warn" }` 并口头提示可阅读 `prd-harness-options.md` 后改 `strict`。 |
...
**`prd` 段（Visual Handoff 守门）**：须与 Step 2 已定值一致。CREATE 若用户未表态，采用 [templates/framework.config.template.json](../../templates/framework.config.template.json) skeleton 中的默认值 **`prd.visual_handoff_enforcement: "warn"`**；若在 Step 2 已与用户确认 `strict` / `off`，则写入确认值。说明与选型表见 [prompts/prd-harness-options.md](prompts/prd-harness-options.md)。
...
| PRD harness：`prd.visual_handoff_enforcement` 选型 | [prompts/prd-harness-options.md](prompts/prd-harness-options.md) |
