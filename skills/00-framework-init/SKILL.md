...
> 可选辅助工具：[framework/harness/scripts/render-agents-md.mjs](../../harness/scripts/render-agents-md.mjs) 一次性完成上表占位符替换并写入目标路径，避免逐项手写差错。AI 仍可手工渲染，两者等价；选用脚本时务必确保脚本的 `vars` 表与上表字段完全一致（脚本启动会自检剩余 `{{...}}` 占位符并失败退出）。
