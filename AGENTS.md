...
AgentMaison 的“效率优先”“简单优先”“回退重签”“协作可恢复”四条总原则以 [`docs/overview.md §1.2.1`](docs/overview.md#121-四条总设计原则) 为 SSOT。框架设计、plan 与 review 必须同时遵守；不得为追求过程证据完美或理论最优牺牲交付效率，不得另建平行版本、用局部便利绕过权责阶段，或以防篡改之名把可恢复的协作失误升级为常态钥匙/人工门禁。

## Review 要求（简单、高效、可靠）

- 首轮冻结本批范围、责任归属、最小修复与验收标准，一次性报告可发现的阻断问题；不把后续 plan 的未实施内容算作本批缺陷。
- 沿实际消费链判断缺陷，区分输入可解析、能力可用与业务验收通过。现有责任方已充分检查时，不为“接口更严格”前移或重复校验；假设性风险与可选加固不得升级为阻断。
- 每条意见给出严重级别、文件定位、具体触发与影响、最小修复边界及必要反例。优先复用既有解析、真源与恢复机制，完整解决根因，不借 review 增加无必要的状态、协议、门禁或框架。
- 返修后只核既有意见和直接回归；新增阻断须有具体新证据并说明此前为何无法合理发现。判断过重时明确撤回，完整更新“必须修复 / 非阻断建议 / 撤回意见”，不让实施方继续执行旧意见。
- 验证与实际改动风险相称，复用仍有效结果，不因 review 轮次增加而重复全测；仓库明确要求的门禁仍须满足。既有问题修复且无直接回归时明确通过、及时收口。
- 除非用户明确要求，review 只在回复正文输出普通文件链接，不写行内评论、不直接修改被审实现。
