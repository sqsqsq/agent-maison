---
name: goal-mode
description: 目标模式生产入口（有人在场先 prepare-run 再走 goal-mode-entry host bridge，无人值守走 detached runner）
---
...
<!--
  ...
  - 非 Claude 宿主：init syncTemplateTarget 对 goal-mode/SKILL.md 改调
    ...
    程序化生成 adapter 中性的 stub；运行身份只取 personal setup 的 local-first 结果。
  - Claude 宿主：builtin slash 走 agents/claude/templates/commands/goal-mode.md 静态模板，与本文件无关。
  - 改跳板文案 → 改上述生成器；勿改本文件 expecting 生效。
-->
