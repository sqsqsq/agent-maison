...
完整读一遍 [framework/skills/1-prd-design/SKILL.md](../../framework/skills/1-prd-design/SKILL.md)，按其中的 Step 0 → Step N 严格执行，产物路径、harness 命令、完成标准、BLOCKER 清单**全部以 SKILL.md 原文为准**。
...
结束本次 `/prd-design` 前，主 agent **必须亲手**做完：

1. **自跑脚本 harness**：`cd framework/harness && npx ts-node harness-runner.ts --phase prd --feature <name>`
...
3. **填写并校验完成回执**：
   ...
   - 输出：`doc/features/<name>/prd/phase-completion-receipt.md`
   - 校验：`npx ts-node framework/harness/scripts/check-receipt.ts --feature <name> --phase prd`
